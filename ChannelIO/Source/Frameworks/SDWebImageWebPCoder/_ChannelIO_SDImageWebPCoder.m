...
static CGSize _ChannelIO_SDCalculateThumbnailSize(CGSize fullSize, BOOL preserveAspectRatio, CGSize thumbnailSize) {
...
@interface _ChannelIO_SDWebPCoderFrame : NSObject
...
@implementation _ChannelIO_SDWebPCoderFrame
@end

@implementation _ChannelIO_SDImageWebPCoder {
    WebPIDecoder *_idec;
    WebPDemuxer *_demux;
    NSData *_imageData;
    CGFloat _scale;
    ...
    NSArray<_ChannelIO_SDWebPCoderFrame *> *_frames;
...
+ (instancetype)sharedCoder {
    static _ChannelIO_SDImageWebPCoder *coder;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coder = [[_ChannelIO_SDImageWebPCoder alloc] init];
    });
    return coder;
}
...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    return ([NSData _ChannelIO_sd_imageFormatForImageData:data] == _ChannelIO_SDImageFormatWebP);
}
...
- (UIImage *)decodedImageWithData:(NSData *)data options:(nullable _ChannelIO_SDImageCoderOptions *)options {
    ...
    BOOL decodeFirstFrame = [options[_ChannelIO_SDImageCoderDecodeFirstFrameOnly] boolValue];
    CGFloat scale = 1;
    NSNumber *scaleFactor = options[_ChannelIO_SDImageCoderDecodeScaleFactor];
    if (scaleFactor != nil) {
    ...
    NSValue *thumbnailSizeValue = options[_ChannelIO_SDImageCoderDecodeThumbnailPixelSize];
    ...
    NSNumber *preserveAspectRatioValue = options[_ChannelIO_SDImageCoderDecodePreserveAspectRatio];
    ...
    CGSize scaledSize = _ChannelIO_SDCalculateThumbnailSize(CGSizeMake(canvasWidth, canvasHeight), preserveAspectRatio, thumbnailSize);
...
#endif
        firstFrameImage._ChannelIO_sd_imageFormat = _ChannelIO_SDImageFormatWebP;
    ...
    CGContextRef canvas = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8, 0, [_ChannelIO_SDImageCoderHelper colorSpaceGetDeviceRGB], bitmapInfo);
    if (!canvas) {
        WebPDemuxDelete(demuxer);
        CGColorSpaceRelease(colorSpace);
        return nil;
    ...
    NSMutableArray<_ChannelIO_SDImageFrame *> *frames = [NSMutableArray array];
    ...
    animatedImage._ChannelIO_sd_imageLoopCount = loopCount;
    animatedImage._ChannelIO_sd_imageFormat = _ChannelIO_SDImageFormatWebP;
    
    return animatedImage;
}

#pragma mark - Progressive Decode
- (instancetype)initIncrementalWithOptions:(nullable _ChannelIO_SDImageCoderOptions *)options {
    self = [super init];
    if (self) {
...
- (UIImage *)incrementalDecodedImageWithOptions:(_ChannelIO_SDImageCoderOptions *)options {
    UIImage *image;
    ...
    if (_demux) {
        ...
        SD_UNLOCK(_lock);
        image._ChannelIO_sd_imageFormat = _ChannelIO_SDImageFormatWebP;
        image._ChannelIO_sd_isDecoded = YES;
        return image;
    }
    ...
    if (width + height > 0 && last_y > 0 && height >= last_y) {
        ...
        CGColorSpaceRef colorSpaceRef = [_ChannelIO_SDImageCoderHelper colorSpaceGetDeviceRGB];
        ...
        CGContextRef canvas = CGBitmapContextCreate(NULL, width, height, 8, 0, [_ChannelIO_SDImageCoderHelper colorSpaceGetDeviceRGB], bitmapInfo);
        if (!canvas) {
            CGImageRelease(imageRef);
            return nil;
        }
        ...
        CGSize scaledSize = _ChannelIO_SDCalculateThumbnailSize(CGSizeMake(width, height), _preserveAspectRatio, _thumbnailSize);
        // Check whether we need to use thumbnail
        if (!CGSizeEqualToSize(CGSizeMake(width, height), scaledSize)) {
            CGImageRef scaledImageRef = [_ChannelIO_SDImageCoderHelper CGImageCreateScaled:newImageRef size:scaledSize];
...
#endif
        image._ChannelIO_sd_isDecoded = YES; // Already drawn on bitmap context above
        image._ChannelIO_sd_imageFormat = _ChannelIO_SDImageFormatWebP;
        CGImageRelease(newImageRef);
        CGContextRelease(canvas);
    }
...
- (nonnull CGColorSpaceRef)sd_createColorSpaceWithDemuxer:(nonnull WebPDemuxer *)demuxer CF_RETURNS_RETAINED {
    ...
    if (!colorSpaceRef) {
        colorSpaceRef = [_ChannelIO_SDImageCoderHelper colorSpaceGetDeviceRGB];
...
- (BOOL)canEncodeToFormat:(_ChannelIO_SDImageFormat)format {
    return (format == _ChannelIO_SDImageFormatWebP);
}
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(_ChannelIO_SDImageFormat)format options:(nullable _ChannelIO_SDImageCoderOptions *)options {
    ...
    if (options[_ChannelIO_SDImageCoderEncodeCompressionQuality]) {
        compressionQuality = [options[_ChannelIO_SDImageCoderEncodeCompressionQuality] doubleValue];
    }
    CGSize maxPixelSize = CGSizeZero;
    NSValue *maxPixelSizeValue = options[_ChannelIO_SDImageCoderEncodeMaxPixelSize];
    ...
    if (options[_ChannelIO_SDImageCoderEncodeMaxFileSize]) {
        maxFileSize = [options[_ChannelIO_SDImageCoderEncodeMaxFileSize] unsignedIntegerValue];
    }
    NSArray<_ChannelIO_SDImageFrame *> *frames = [_ChannelIO_SDImageCoderHelper framesFromAnimatedImage:image];
    
    BOOL encodeFirstFrame = [options[_ChannelIO_SDImageCoderEncodeFirstFrameOnly] boolValue];
    if (encodeFirstFrame || frames.count == 0) {
        ...
        for (size_t i = 0; i < frames.count; i++) {
            _ChannelIO_SDImageFrame *currentFrame = frames[i];
        ...
        int loopCount = (int)image._ChannelIO_sd_imageLoopCount;
...
- (nullable NSData *)sd_encodedWebpDataWithImage:(nullable CGImageRef)imageRef quality:(double)quality maxPixelSize:(CGSize)maxPixelSize maxFileSize:(NSUInteger)maxFileSize {
    ...
    if (byteOrderNormal && ((alphaInfo == kCGImageAlphaNone) || (alphaInfo == kCGImageAlphaLast))) {
        ...
        vImage_CGImageFormat destFormat = {
            ...
            .colorSpace = [_ChannelIO_SDImageCoderHelper colorSpaceGetDeviceRGB],
    ...
    if (maxPixelSize.width > 0 && maxPixelSize.height > 0 && width > maxPixelSize.width && height > maxPixelSize.height) {
        CGSize scaledSize = _ChannelIO_SDCalculateThumbnailSize(CGSizeMake(width, height), YES, maxPixelSize);
...
- (instancetype)initWithAnimatedImageData:(NSData *)data options:(nullable _ChannelIO_SDImageCoderOptions *)options {
    if (!data) {
        return nil;
    }
    if (self) {
...
- (BOOL)scanAndCheckFramesValidWithDemuxer:(WebPDemuxer *)demuxer {
    ...
    NSMutableArray<_ChannelIO_SDWebPCoderFrame *> *frames = [NSMutableArray array];
    ...
    do {
        ...
        _ChannelIO_SDWebPCoderFrame *frame = [[_ChannelIO_SDWebPCoderFrame alloc] init];
...
- (UIImage *)safeStaticImageFrame {
    ...
    if (_hasAnimation) {
        ...
        if (!_canvas) {
            ...
            CGContextRef canvas = CGBitmapContextCreate(NULL, _canvasWidth, _canvasHeight, 8, 0, [_ChannelIO_SDImageCoderHelper colorSpaceGetDeviceRGB], bitmapInfo);
            if (!canvas) {
                return nil;
            }
            _canvas = canvas;
        }
        CGSize scaledSize = _ChannelIO_SDCalculateThumbnailSize(CGSizeMake(_canvasWidth, _canvasHeight), _preserveAspectRatio, _thumbnailSize);
        imageRef = [self sd_drawnWebpImageWithCanvas:_canvas iterator:iter colorSpace:_colorSpace scaledSize:scaledSize];
    } else {
        CGSize scaledSize = _ChannelIO_SDCalculateThumbnailSize(CGSizeMake(iter.width, iter.height), _preserveAspectRatio, _thumbnailSize);
...
- (UIImage *)safeAnimatedImageFrameAtIndex:(NSUInteger)index {
    ...
    _ChannelIO_SDWebPCoderFrame *frame = _frames[index];
    UIImage *image;
    WebPIterator iter;
