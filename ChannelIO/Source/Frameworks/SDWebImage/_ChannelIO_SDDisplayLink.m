...
#if SD_MAC
static CVReturn _ChannelIO_DisplayLinkCallback(CVDisplayLinkRef displayLink, const CVTimeStamp *inNow, const CVTimeStamp *inOutputTime, CVOptionFlags flagsIn, CVOptionFlags *flagsOut, void *displayLinkContext);
#endif

#define kSDDisplayLinkInterval 1.0 / 60

@interface _ChannelIO_SDDisplayLink ()
...
@implementation _ChannelIO_SDDisplayLink
...
#elif SD_IOS || SD_TV
        _ChannelIO_SDWeakProxy *weakProxy = [_ChannelIO_SDWeakProxy proxyWithTarget:self];
...
+ (instancetype)displayLinkWithTarget:(id)target selector:(SEL)sel {
    _ChannelIO_SDDisplayLink *displayLink = [[_ChannelIO_SDDisplayLink alloc] initWithTarget:target selector:sel];
    return displayLink;
}
