...
static NSArray *_ChannelIO_SDBundlePreferredScales() {
...
@implementation _ChannelIO_SDImageAssetManager {
    dispatch_semaphore_t _lock;
}

+ (instancetype)sharedAssetManager {
    static dispatch_once_t onceToken;
    static _ChannelIO_SDImageAssetManager *assetManager;
    dispatch_once(&onceToken, ^{
        assetManager = [[_ChannelIO_SDImageAssetManager alloc] init];
    });
    return assetManager;
}
...
- (NSString *)getPathForName:(NSString *)name bundle:(NSBundle *)bundle preferredScale:(CGFloat *)scale {
    ...
    NSArray *scales = _ChannelIO_SDBundlePreferredScales();
