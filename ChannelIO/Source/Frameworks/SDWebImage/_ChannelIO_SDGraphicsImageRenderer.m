...
@interface _ChannelIO_SDGraphicsImageRendererFormat ()
#if SD_UIKIT
@property (nonatomic, strong) UIGraphicsImageRendererFormat *uiformat API_AVAILABLE(ios(10.0), tvos(10.0));
#endif
@end

@implementation _ChannelIO_SDGraphicsImageRendererFormat
...
- (_ChannelIO_SDGraphicsImageRendererFormatRange)preferredRange {
#if SD_UIKIT
    if (@available(iOS 10.0, tvOS 10.10, *)) {
        if (@available(iOS 12.0, tvOS 12.0, *)) {
            return (_ChannelIO_SDGraphicsImageRendererFormatRange)self.uiformat.preferredRange;
        } else {
...
- (void)setPreferredRange:(_ChannelIO_SDGraphicsImageRendererFormatRange)preferredRange {
#if SD_UIKIT
    if (@available(iOS 10.0, tvOS 10.10, *)) {
        if (@available(iOS 12.0, tvOS 12.0, *)) {
...
+ (instancetype)preferredFormat {
    _ChannelIO_SDGraphicsImageRendererFormat *format = [[_ChannelIO_SDGraphicsImageRendererFormat alloc] initForMainScreen];
    return format;
}

@end

@interface _ChannelIO_SDGraphicsImageRenderer ()
@property (nonatomic, assign) CGSize size;
@property (nonatomic, strong) _ChannelIO_SDGraphicsImageRendererFormat *format;
#if SD_UIKIT
@property (nonatomic, strong) UIGraphicsImageRenderer *uirenderer API_AVAILABLE(ios(10.0), tvos(10.0));
#endif
@end

@implementation _ChannelIO_SDGraphicsImageRenderer

- (instancetype)initWithSize:(CGSize)size {
    return [self initWithSize:size format:_ChannelIO_SDGraphicsImageRendererFormat.preferredFormat];
}

- (instancetype)initWithSize:(CGSize)size format:(_ChannelIO_SDGraphicsImageRendererFormat *)format {
...
- (UIImage *)imageWithActions:(NS_NOESCAPE _ChannelIO_SDGraphicsImageDrawingActions)actions {
...
#endif
        _ChannelIO_SDGraphicsBeginImageContextWithOptions(self.size, self.format.opaque, self.format.scale);
        CGContextRef context = _ChannelIO_SDGraphicsGetCurrentContext();
        if (actions) {
            actions(context);
        }
        UIImage *image = _ChannelIO_SDGraphicsGetImageFromCurrentImageContext();
