...
static NSString * _ChannelIO_kSDCGImageSourceRasterizationDPI = @"kCGImageSourceRasterizationDPI";
// Specify File Size for lossy format encoding, like JPEG
static NSString * _ChannelIO_kSDCGImageDestinationRequestedFileSize = @"kCGImageDestinationRequestedFileSize";

@interface _ChannelIO_SDImageIOCoderFrame : NSObject
...
@implementation _ChannelIO_SDImageIOCoderFrame
@end

@implementation _ChannelIO_SDImageIOAnimatedCoder {
    ...
    NSArray<_ChannelIO_SDImageIOCoderFrame *> *_frames;
    BOOL _finished;
...
+ (BOOL)canDecodeFromFormat:(_ChannelIO_SDImageFormat)format {
    ...
    CFStringRef imageUTType = [NSData _ChannelIO_sd_UTTypeFromImageFormat:format];
...
+ (UIImage *)createFrameAtIndex:(NSUInteger)index source:(CGImageSourceRef)source scale:(CGFloat)scale preserveAspectRatio:(BOOL)preserveAspectRatio thumbnailSize:(CGSize)thumbnailSize options:(NSDictionary *)options {
    ...
    // Check vector format
    BOOL isVector = NO;
    if ([NSData _ChannelIO_sd_imageFormatFromUTType:uttype] == _ChannelIO_SDImageFormatPDF) {
        isVector = YES;
    }
    ...
    if (createFullImage) {
        if (isVector) {
            ...
            decodingOptions[_ChannelIO_kSDCGImageSourceRasterizationDPI] = @(rasterizationDPI);
        }
    ...
    if (!createFullImage) {
        if (preserveAspectRatio) {
            ...
            CGImageRef scaledImageRef = [_ChannelIO_SDImageCoderHelper CGImageCreateScaled:imageRef size:thumbnailSize];
    ...
    UIImageOrientation imageOrientation = [_ChannelIO_SDImageCoderHelper imageOrientationFromEXIFOrientation:exifOrientation];
...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    return ([NSData _ChannelIO_sd_imageFormatForImageData:data] == self.class.imageFormat);
}
...
- (UIImage *)decodedImageWithData:(NSData *)data options:(nullable _ChannelIO_SDImageCoderOptions *)options {
    ...
    if (decodeFirstFrame || count <= 1) {
        ...
        animatedImage = [_ChannelIO_SDImageCoderHelper animatedImageWithFrames:frames];
        animatedImage._ChannelIO_sd_imageLoopCount = loopCount;
    }
    animatedImage._ChannelIO_sd_imageFormat = self.class.imageFormat;
    CFRelease(source);
...
- (UIImage *)incrementalDecodedImageWithOptions:(_ChannelIO_SDImageCoderOptions *)options {
    ...
    if (_width + _height > 0) {
        ...
        if (image) {
            image._ChannelIO_sd_imageFormat = self.class.imageFormat;
        }
    }
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(_ChannelIO_SDImageFormat)format options:(nullable _ChannelIO_SDImageCoderOptions *)options {
    ...
    CGColorRef backgroundColor = [options[_ChannelIO_SDImageCoderEncodeBackgroundColor] CGColor];
    if (backgroundColor) {
    ...
    NSUInteger maxFileSize = [options[_ChannelIO_SDImageCoderEncodeMaxFileSize] unsignedIntegerValue];
    if (maxFileSize > 0) {
        properties[_ChannelIO_kSDCGImageDestinationRequestedFileSize] = @(maxFileSize);
    ...
    if (options[_ChannelIO_SDImageCoderEncodeEmbedThumbnail]) {
        embedThumbnail = [options[_ChannelIO_SDImageCoderEncodeEmbedThumbnail] boolValue];
    }
    ...
    if (encodeFirstFrame || frames.count == 0) {
        ...
        // for animated images
        NSUInteger loopCount = image._ChannelIO_sd_imageLoopCount;
        ...
        for (size_t i = 0; i < frames.count; i++) {
            _ChannelIO_SDImageFrame *frame = frames[i];
...
- (nullable instancetype)initWithAnimatedImageData:(nullable NSData *)data options:(nullable _ChannelIO_SDImageCoderOptions *)options {
    if (!data) {
        return nil;
    }
    self = [super init];
...
- (BOOL)scanAndCheckFramesValidWithImageSource:(CGImageSourceRef)imageSource {
    ...
    NSMutableArray<_ChannelIO_SDImageIOCoderFrame *> *frames = [NSMutableArray array];
    
    for (size_t i = 0; i < frameCount; i++) {
        _ChannelIO_SDImageIOCoderFrame *frame = [[_ChannelIO_SDImageIOCoderFrame alloc] init];
        frame.index = i;
