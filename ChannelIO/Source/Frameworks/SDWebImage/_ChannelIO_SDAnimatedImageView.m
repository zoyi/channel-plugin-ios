...
@interface _ChannelIO_SDAnimatedImageView () {
...
@property (nonatomic,strong) _ChannelIO_SDAnimatedImagePlayer *player; // The animation player.
@property (nonatomic) CALayer *imageViewLayer; // The actual rendering layer.

@end

@implementation _ChannelIO_SDAnimatedImageView
#if SD_UIKIT
...
- (void)setImage:(UIImage *)image
{
    ...
    if ([image.class conformsToProtocol:@protocol(_ChannelIO_SDAnimatedImage)]) {
        if (!self.player) {
            id<_ChannelIO_SDAnimatedImageProvider> provider;
            // Check progressive loading
            if (self.isProgressive) {
                provider = [self progressiveAnimatedCoderForImage:image];
            } else {
                provider = (id<_ChannelIO_SDAnimatedImage>)image;
            }
            // Create animated player
            self.player = [_ChannelIO_SDAnimatedImagePlayer playerWithProvider:provider];
        } else {
            // Update Frame Count
            self.player.totalFrameCount = [(id<_ChannelIO_SDAnimatedImage>)image animatedImageFrameCount];
        }
...
- (void)updateIsProgressiveWithImage:(UIImage *)image
{
    ...
    id<_ChannelIO_SDAnimatedImageCoder> currentAnimatedCoder = [self progressiveAnimatedCoderForImage:image];
    if (currentAnimatedCoder) {
        ...
        if (!previousImage) {
            ...
            id<_ChannelIO_SDAnimatedImageCoder> previousAnimatedCoder = [self progressiveAnimatedCoderForImage:previousImage];
...
- (id<_ChannelIO_SDAnimatedImageCoder, _ChannelIO_SDProgressiveImageCoder>)progressiveAnimatedCoderForImage:(UIImage *)image
{
    if ([image.class conformsToProtocol:@protocol(_ChannelIO_SDAnimatedImage)] && image._ChannelIO_sd_isIncremental && [image respondsToSelector:@selector(animatedCoder)]) {
        id<_ChannelIO_SDAnimatedImageCoder> animatedCoder = [(id<_ChannelIO_SDAnimatedImage>)image animatedCoder];
        if ([animatedCoder conformsToProtocol:@protocol(_ChannelIO_SDProgressiveImageCoder)]) {
            return (id<_ChannelIO_SDAnimatedImageCoder, _ChannelIO_SDProgressiveImageCoder>)animatedCoder;
        }
    }
    return nil;
}
