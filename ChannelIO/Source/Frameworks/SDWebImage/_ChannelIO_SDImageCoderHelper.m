...
static inline size_t _ChannelIO_SDByteAlign(size_t size, size_t alignment) {
    return ((size + (alignment - 1)) / alignment) * alignment;
}

static const size_t _ChannelIO_kBytesPerPixel = 4;
static const size_t _ChannelIO_kBitsPerComponent = 8;

static const CGFloat _ChannelIO_kBytesPerMB = 1024.0f * 1024.0f;
/*
 ...
 */
#if SD_MAC
static CGFloat _ChannelIO_kDestImageLimitBytes = 90.f * _ChannelIO_kBytesPerMB;
#elif SD_UIKIT
static CGFloat _ChannelIO_kDestImageLimitBytes = 60.f * _ChannelIO_kBytesPerMB;
#elif SD_WATCH
static CGFloat _ChannelIO_kDestImageLimitBytes = 30.f * _ChannelIO_kBytesPerMB;
#endif

static const CGFloat _ChannelIO_kDestSeemOverlap = 2.0f;   // the numbers of pixels to overlap the seems where tiles meet.

@implementation _ChannelIO_SDImageCoderHelper

+ (UIImage *)animatedImageWithFrames:(NSArray<_ChannelIO_SDImageFrame *> *)frames {
    ...
    [frames enumerateObjectsUsingBlock:^(_ChannelIO_SDImageFrame * _Nonnull frame, NSUInteger idx, BOOL * _Nonnull stop) {
...
+ (NSArray<_ChannelIO_SDImageFrame *> *)framesFromAnimatedImage:(UIImage *)animatedImage {
    if (!animatedImage) {
        return nil;
    }
    ...
    [animatedImages enumerateObjectsUsingBlock:^(UIImage * _Nonnull image, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([image isEqual:previousImage]) {
            ...
            _ChannelIO_SDImageFrame *frame = [_ChannelIO_SDImageFrame frameWithImage:previousImage duration:avgDuration * repeatCount];
...
+ (CGImageRef)CGImageCreateScaled:(CGImageRef)cgImage size:(CGSize)size {
    ...
    output_buffer.rowBytes = _ChannelIO_SDByteAlign(output_buffer.width * 4, 64);
...
+ (UIImage *)decodedImageWithImage:(UIImage *)image {
    ...
    _ChannelIO_SDImageCopyAssociatedObject(image, decodedImage);
    decodedImage._ChannelIO_sd_isDecoded = YES;
    return decodedImage;
}
...
+ (UIImage *)decodedAndScaledDownImageWithImage:(UIImage *)image limitBytes:(NSUInteger)bytes {
    ...
    if (bytes == 0) {
        bytes = _ChannelIO_kDestImageLimitBytes;
    }
    destTotalPixels = bytes / _ChannelIO_kBytesPerPixel;
    ...
    @autoreleasepool {
        ...
        float sourceSeemOverlap = (int)((_ChannelIO_kDestSeemOverlap/destResolution.height)*sourceResolution.height);
        ...
        destTile.size.height += _ChannelIO_kDestSeemOverlap;
        for( int y = 0; y < iterations; ++y ) {
            @autoreleasepool {
                sourceTile.origin.y = y * sourceTileHeightMinusOverlap + sourceSeemOverlap;
                destTile.origin.y = destResolution.height - (( y + 1 ) * sourceTileHeightMinusOverlap * imageScale + _ChannelIO_kDestSeemOverlap);
        ...
        _ChannelIO_SDImageCopyAssociatedObject(image, destImage);
        destImage._ChannelIO_sd_isDecoded = YES;
        return destImage;
    }
}

+ (NSUInteger)defaultScaleDownLimitBytes {
    return _ChannelIO_kDestImageLimitBytes;
}

+ (void)setDefaultScaleDownLimitBytes:(NSUInteger)defaultScaleDownLimitBytes {
    if (defaultScaleDownLimitBytes < _ChannelIO_kBytesPerPixel) {
        return;
    }
    _ChannelIO_kDestImageLimitBytes = defaultScaleDownLimitBytes;
}
...
+ (BOOL)shouldDecodeImage:(nullable UIImage *)image {
    ...
    // Avoid extra decode
    if (image._ChannelIO_sd_isDecoded) {
        return NO;
    }
    ...
    if (image._ChannelIO_sd_isVector) {
        return NO;
    }
...
+ (BOOL)shouldScaleDownImage:(nonnull UIImage *)image limitBytes:(NSUInteger)bytes {
    ...
    bytes = MAX(bytes, _ChannelIO_kBytesPerPixel);
    destTotalPixels = bytes / _ChannelIO_kBytesPerPixel;
