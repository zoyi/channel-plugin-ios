...
typedef NSData * _Nullable (^_ChannelIO_SDWebImageDownloaderDecryptorBlock)(NSData * _Nonnull data, NSURLResponse * _Nullable response);
...
*/
@protocol _ChannelIO_SDWebImageDownloaderDecryptor <NSObject>
...
*/
@interface _ChannelIO_SDWebImageDownloaderDecryptor : NSObject <_ChannelIO_SDWebImageDownloaderDecryptor>

/// Create the data decryptor with block
/// @param block A block to control decrypt logic
- (nonnull instancetype)initWithBlock:(nonnull _ChannelIO_SDWebImageDownloaderDecryptorBlock)block;

/// Create the data decryptor with block
/// @param block A block to control decrypt logic
+ (nonnull instancetype)decryptorWithBlock:(nonnull _ChannelIO_SDWebImageDownloaderDecryptorBlock)block;

@end

/// Convenience way to create decryptor for common data encryption.
@interface _ChannelIO_SDWebImageDownloaderDecryptor (Conveniences)

/// Base64 Encoded image data decryptor
@property (class, readonly, nonnull) _ChannelIO_SDWebImageDownloaderDecryptor *base64Decryptor;
