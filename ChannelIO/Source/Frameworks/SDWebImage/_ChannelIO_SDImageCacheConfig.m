...
static _ChannelIO_SDImageCacheConfig *_ChannelIO__defaultCacheConfig;
static const NSInteger kDefaultCacheMaxDiskAge = 60 * 60 * 24 * 7; // 1 week

@implementation _ChannelIO_SDImageCacheConfig

+ (_ChannelIO_SDImageCacheConfig *)defaultCacheConfig {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _ChannelIO__defaultCacheConfig = [_ChannelIO_SDImageCacheConfig new];
    });
    return _ChannelIO__defaultCacheConfig;
}
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        _memoryCacheClass = [_ChannelIO_SDMemoryCache class];
        _diskCacheClass = [_ChannelIO_SDDiskCache class];
    }
    return self;
}

- (id)copyWithZone:(NSZone *)zone {
    _ChannelIO_SDImageCacheConfig *config = [[[self class] allocWithZone:zone] init];
