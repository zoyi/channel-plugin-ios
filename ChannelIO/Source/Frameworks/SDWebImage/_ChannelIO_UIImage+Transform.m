...
static inline CGRect _ChannelIO_SDCGRectFitWithScaleMode(CGRect rect, CGSize size, _ChannelIO_SDImageScaleMode scaleMode) {
...
static inline UIColor * _ChannelIO_SDGetColorFromPixel(Pixel_8888 pixel, CGBitmapInfo bitmapInfo) {
...
static inline CGImageRef _Nullable _ChannelIO_SDCreateCGImageFromCIImage(CIImage * _Nonnull ciImage) {
...
@implementation UIImage (_ChannelIO_Transform)

- (void)_ChannelIO_sd_drawInRect:(CGRect)rect context:(CGContextRef)context scaleMode:(_ChannelIO_SDImageScaleMode)scaleMode clipsToBounds:(BOOL)clips {
    CGRect drawRect = _ChannelIO_SDCGRectFitWithScaleMode(rect, self.size, scaleMode);
...
- (nullable UIImage *)_ChannelIO_sd_resizedImageWithSize:(CGSize)size scaleMode:(_ChannelIO_SDImageScaleMode)scaleMode {
    if (size.width <= 0 || size.height <= 0) return nil;
    _ChannelIO_SDGraphicsImageRendererFormat *format = [[_ChannelIO_SDGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    _ChannelIO_SDGraphicsImageRenderer *renderer = [[_ChannelIO_SDGraphicsImageRenderer alloc] initWithSize:size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
        [self _ChannelIO_sd_drawInRect:CGRectMake(0, 0, size.width, size.height) context:context scaleMode:scaleMode clipsToBounds:NO];
    }];
    return image;
}

- (nullable UIImage *)_ChannelIO_sd_croppedImageWithRect:(CGRect)rect {
...
- (nullable UIImage *)_ChannelIO_sd_roundedCornerImageWithRadius:(CGFloat)cornerRadius corners:(SDRectCorner)corners borderWidth:(CGFloat)borderWidth borderColor:(nullable UIColor *)borderColor {
    _ChannelIO_SDGraphicsImageRendererFormat *format = [[_ChannelIO_SDGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    _ChannelIO_SDGraphicsImageRenderer *renderer = [[_ChannelIO_SDGraphicsImageRenderer alloc] initWithSize:self.size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
...
- (nullable UIImage *)_ChannelIO_sd_rotatedImageWithAngle:(CGFloat)angle fitSize:(BOOL)fitSize {
    ...
    _ChannelIO_SDGraphicsImageRendererFormat *format = [[_ChannelIO_SDGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    _ChannelIO_SDGraphicsImageRenderer *renderer = [[_ChannelIO_SDGraphicsImageRenderer alloc] initWithSize:newRect.size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
...
- (nullable UIImage *)_ChannelIO_sd_flippedImageWithHorizontal:(BOOL)horizontal vertical:(BOOL)vertical {
    size_t width = self.size.width;
    size_t height = self.size.height;
...
- (nullable UIImage *)_ChannelIO_sd_tintedImageWithColor:(nonnull UIColor *)tintColor {
...
- (nullable UIColor *)_ChannelIO_sd_colorAtPoint:(CGPoint)point {
    CGImageRef imageRef = NULL;
    // CIImage compatible
#if SD_UIKIT || SD_MAC
    if (self.CIImage) {
        imageRef = _ChannelIO_SDCreateCGImageFromCIImage(self.CIImage);
    }
#endif
    if (!imageRef) {
    ...
    CGImageRelease(imageRef);
    // Convert to color
    return _ChannelIO_SDGetColorFromPixel(pixel, bitmapInfo);
}

- (nullable NSArray<UIColor *> *)_ChannelIO_sd_colorsWithRect:(CGRect)rect {
    CGImageRef imageRef = NULL;
    // CIImage compatible
#if SD_UIKIT || SD_MAC
    if (self.CIImage) {
    ...
    for (size_t index = start; index < end; index += 4) {
        ...
        UIColor *color = _ChannelIO_SDGetColorFromPixel(pixel, bitmapInfo);
...
- (nullable UIImage *)_ChannelIO_sd_blurredImageWithRadius:(CGFloat)blurRadius {
    ...
    if (CGImageGetBitsPerPixel(imageRef) != 32 ||
        ...
        _ChannelIO_SDGraphicsBeginImageContextWithOptions(self.size, NO, self.scale);
        [self drawInRect:CGRectMake(0, 0, self.size.width, self.size.height)];
        imageRef = _ChannelIO_SDGraphicsGetImageFromCurrentImageContext().CGImage;
        _ChannelIO_SDGraphicsEndImageContext();
    }
...
#if SD_UIKIT || SD_MAC
- (nullable UIImage *)_ChannelIO_sd_filteredImageWithFilter:(nonnull CIFilter *)filter {
    CIImage *inputImage;
    if (self.CIImage) {
