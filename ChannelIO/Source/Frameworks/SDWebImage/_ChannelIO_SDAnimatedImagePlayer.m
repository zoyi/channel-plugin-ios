...
@interface _ChannelIO_SDAnimatedImagePlayer () {
...
@property (nonatomic, strong) id<_ChannelIO_SDAnimatedImageProvider> animatedProvider;
...
@property (nonatomic, strong) _ChannelIO_SDDisplayLink *displayLink;

@end

@implementation _ChannelIO_SDAnimatedImagePlayer

- (instancetype)initWithProvider:(id<_ChannelIO_SDAnimatedImageProvider>)provider {
    self = [super init];
    if (self) {
...
+ (instancetype)playerWithProvider:(id<_ChannelIO_SDAnimatedImageProvider>)provider {
    _ChannelIO_SDAnimatedImagePlayer *player = [[_ChannelIO_SDAnimatedImagePlayer alloc] initWithProvider:provider];
    return player;
}
...
- (_ChannelIO_SDDisplayLink *)displayLink {
    if (!_displayLink) {
        _displayLink = [_ChannelIO_SDDisplayLink displayLinkWithTarget:self selector:@selector(displayDidRefresh:)];
...
- (void)displayDidRefresh:(_ChannelIO_SDDisplayLink *)displayLink {
    ...
    if (!fetchFrame && !bufferFull && self.fetchQueue.operationCount == 0) {
        ...
        id<_ChannelIO_SDAnimatedImageProvider> animatedProvider = self.animatedProvider;
        @weakify(self);
...
- (void)calculateMaxBufferCount {
    ...
    if (self.maxBufferSize > 0) {
        ...
        NSUInteger total = [_ChannelIO_SDDeviceHelper totalMemory];
        NSUInteger free = [_ChannelIO_SDDeviceHelper freeMemory];
