...
extension _RXSwift_Observable where Element: _RXSwift_ObservableType {
	/**
	...
	*/
    static func cascade<Sequence: Swift.Sequence>(_ observables: Sequence) -> _RXSwift_Observable<Element.Element> where Sequence.Element == Element {
        let flow = Array(observables)
        if flow.isEmpty {
            return _RXSwift_Observable<Element.Element>.empty()
        }

        return _RXSwift_Observable<Element.Element>.create { observer in
            var current = 0, initialized = false
            var subscriptions: [_RXSwift_SerialDisposable?] = flow.map { _ in _RXSwift_SerialDisposable() }
...
extension _RXSwift_ObservableType {
    ...
    - returns: An observable sequence that contains elements from the latest observable sequence that emitted elements
    */
    func cascade<S: Sequence>(_ next: S) -> _RXSwift_Observable<Element> where S.Element == Self {
        return _RXSwift_Observable.cascade([self.asObservable()] + Array(next).map { $0.asObservable() })
    }
}
