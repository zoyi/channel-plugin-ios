...
typealias _RXEXt_RetryPredicate = (Error) -> Bool

extension _RXEXt_RepeatBehavior {
	/**
...
extension _RXSwift_ObservableType {
	...
	*/
	...
	func retry(_ behavior: _RXEXt_RepeatBehavior, scheduler: _RXSwift_SchedulerType = _RXSwift_MainScheduler.instance, shouldRetry: _RXEXt_RetryPredicate? = nil) -> _RXSwift_Observable<Element> {
	...
	*/
	...
	internal func retry(_ currentAttempt: UInt, behavior: _RXEXt_RepeatBehavior, scheduler: _RXSwift_SchedulerType = _RXSwift_MainScheduler.instance, shouldRetry: _RXEXt_RetryPredicate? = nil)
		-> _RXSwift_Observable<Element> {
			guard currentAttempt > 0 else { return _RXSwift_Observable.empty() }

			// calculate conditions for bahavior
			let conditions = behavior.calculateConditions(currentAttempt)

			return catchError { error -> _RXSwift_Observable<Element> in
				// return error if exceeds maximum amount of retries
				guard conditions.maxCount > currentAttempt else { return _RXSwift_Observable.error(error) }
