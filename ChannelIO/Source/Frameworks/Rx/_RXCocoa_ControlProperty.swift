...
protocol _RXCocoa_ControlPropertyType : _RXSwift_ObservableType, _RXSwift_ObserverType {

    /// - returns: `ControlProperty` interface
    func asControlProperty() -> _RXCocoa_ControlProperty<Element>
}
...
*/
struct _RXCocoa_ControlProperty<PropertyType> : _RXCocoa_ControlPropertyType {
    typealias Element = PropertyType

    let _values: _RXSwift_Observable<PropertyType>
    let _valueSink: _RXSwift_AnyObserver<PropertyType>
    ...
    /// to property.
    init<Values: _RXSwift_ObservableType, Sink: _RXSwift_ObserverType>(values: Values, valueSink: Sink) where Element == Values.Element, Element == Sink.Element {
        self._values = values.subscribeOn(_RXSwift_ConcurrentMainScheduler.instance)
    ...
    var changed: _RXCocoa_ControlEvent<PropertyType> {
        return _RXCocoa_ControlEvent(events: self._values.skip(1))
    }
...
extension _RXCocoa_ControlPropertyType where Element == String? {
    /// Transforms control property of type `String?` into control property of type `String`.
    var orEmpty: _RXCocoa_ControlProperty<String> {
        let original: _RXCocoa_ControlProperty<String?> = self.asControlProperty()

        let values: _RXSwift_Observable<String> = original._values.map { $0 ?? "" }
        let valueSink: _RXSwift_AnyObserver<String> = original._valueSink.mapObserver { $0 }
        return _RXCocoa_ControlProperty<String>(values: values, valueSink: valueSink)
    }
}
