...
/// Observers can subscribe to the subject to receive the last (or initial) value and all subsequent notifications.
 final class _RXSwift_BehaviorSubject<Element>
    : _RXSwift_Observable<Element>
    , _RXSwift_SubjectType
    , _RXSwift_ObserverType
    , _RXSwift_SynchronizedUnsubscribeType
    , _RXSwift_Cancelable {
     typealias SubjectObserverType = _RXSwift_BehaviorSubject<Element>
    ...
    private var _stoppedEvent: _RXSwift_Event<Element>?
    ...
    /// - returns: Latest value.
     func value() throws -> Element {
        self._lock.lock(); defer { self._lock.unlock() } // {
            if self._isDisposed {
                throw _RXSwift_RxError.disposed(object: self)
            }
    ...
    /// Returns observer interface for subject.
     func asObserver() -> _RXSwift_BehaviorSubject<Element> {
        return self
    }
