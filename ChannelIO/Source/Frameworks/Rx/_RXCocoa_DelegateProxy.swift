...
//    import RxSwift
    ...
    class _RXCocoa_DelegateProxy<P: AnyObject, D>: _RXCocoa__RXDelegateProxy {
        ...
        init<Proxy: _RXCocoa_DelegateProxyType>(parentObject: ParentObject, delegateProxy: Proxy.Type)
            where Proxy: _RXCocoa_DelegateProxy<ParentObject, Delegate>, Proxy.ParentObject == ParentObject, Proxy.Delegate == Delegate {
        ...
        override func _sentMessage(_ selector: Selector, withArguments arguments: [Any]) {
            self._sentMessageForSelector[selector]?.on(.next(arguments))
        }

        override func _methodInvoked(_ selector: Selector, withArguments arguments: [Any]) {
        ...
        func forwardToDelegate() -> Delegate? {
            return _RXCocoa_castOptionalOrFatalError(self._forwardToDelegate)
        }
        ...
        fileprivate func reset() {
            ...
            if maybeCurrentDelegate === self {
                ...
                self._setCurrentDelegateTo(_RXCocoa_castOrFatalError(self), parentObject)
            }
        }
    ...
    private let mainScheduler = _RXSwift_MainScheduler()

    private final class MessageDispatcher {
        private let dispatcher: _RXSwift_PublishSubject<[Any]>
        private let result: _RXSwift_Observable<[Any]>

        fileprivate let selector: Selector

        init<P, D>(selector: Selector, delegateProxy _delegateProxy: _RXCocoa_DelegateProxy<P, D>) {
            weak var weakDelegateProxy = _delegateProxy

            let dispatcher = _RXSwift_PublishSubject<[Any]>()
        ...
        var on: (_RXSwift_Event<[Any]>) -> Void {
        ...
        func asObservable() -> _RXSwift_Observable<[Any]> {
            return self.result
        }
    }
