...
extension _RXCocoa_RxCocoaURLError
...
extension _RXSwift_Reactive where Base: URLSession {
    /**
    ...
    */
    func response(request: URLRequest) -> _RXSwift_Observable<(response: HTTPURLResponse, data: Data)> {
        return _RXSwift_Observable.create { observer in

            // smart compiler should be able to optimize this out
            let d: Date?

            if _RXCocoa_Logging.URLRequests(request) {
                d = Date()
            }
            else {
               d = nil
            ...
            let task = self.base.dataTask(with: request) { data, response, error in
                ...
                guard let response = response, let data = data else {
                    observer.on(.error(error ?? _RXCocoa_RxCocoaURLError.unknown))
                    return
                }

                guard let httpResponse = response as? HTTPURLResponse else {
                    observer.on(.error(_RXCocoa_RxCocoaURLError.nonHTTPResponse(response: response)))
                    return
                }
            ...
            return _RXSwift_Disposables.create(with: task.cancel)
        }
    }
    ...
    */
    func data(request: URLRequest) -> _RXSwift_Observable<Data> {
        return self.response(request: request).map { pair -> Data in
            if 200 ..< 300 ~= pair.0.statusCode {
                return pair.1
            }
            else {
                throw _RXCocoa_RxCocoaURLError.httpRequestFailed(response: pair.0, data: pair.1)
            }
        }
    }
    ...
    */
    func json(request: URLRequest, options: JSONSerialization.ReadingOptions = []) -> _RXSwift_Observable<Any> {
        return self.data(request: request).map { data -> Any in
            do {
                return try JSONSerialization.jsonObject(with: data, options: options)
            } catch let error {
                throw _RXCocoa_RxCocoaURLError.deserializationError(error: error)
            }
        }
    }
    ...
    **URL requests will be performed per subscribed observer.**
    ...
    - returns: Observable sequence of response JSON.
    */
    func json(url: Foundation.URL) -> _RXSwift_Observable<Any> {
