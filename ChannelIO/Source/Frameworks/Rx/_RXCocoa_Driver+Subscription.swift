...
extension _RXSwift_SharedSequenceConvertibleType where SharingStrategy == _RXCocoa_DriverSharingStrategy {
    ...
    */
    func drive<Observer: _RXSwift_ObserverType>(_ observer: Observer) -> _RXSwift_Disposable where Observer.Element == Element {
    ...
    */
    func drive(_ relay: _RXRelay_BehaviorRelay<Element>) -> _RXSwift_Disposable {
        _RXSwift_MainScheduler.ensureRunningOnMainThread(errorMessage: errorMessage)
    ...
    */
    func drive<Result>(_ transformation: (_RXSwift_Observable<Element>) -> Result) -> Result {
        _RXSwift_MainScheduler.ensureRunningOnMainThread(errorMessage: errorMessage)
    ...
    - returns: Object representing subscription.
    */
    func drive<R1, R2>(_ with: (_RXSwift_Observable<Element>) -> (R1) -> R2, curriedArgument: R1) -> R2 {
        _RXSwift_MainScheduler.ensureRunningOnMainThread(errorMessage: errorMessage)
    ...
    */
    func drive(onNext: ((Element) -> Void)? = nil, onCompleted: (() -> Void)? = nil, onDisposed: (() -> Void)? = nil) -> _RXSwift_Disposable {
        _RXSwift_MainScheduler.ensureRunningOnMainThread(errorMessage: errorMessage)
