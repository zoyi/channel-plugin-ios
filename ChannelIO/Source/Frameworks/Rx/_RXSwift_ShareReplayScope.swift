...
/// Subject lifetime scope
 enum _RXSwift_SubjectLifetimeScope {
    /**
...
extension _RXSwift_ObservableType {
     ...
     */
     func share(replay: Int = 0, scope: _RXSwift_SubjectLifetimeScope = .whileConnected)
        -> _RXSwift_Observable<Element> {
        switch scope {
        case .forever:
            switch replay {
            case 0: return self.multicast(_RXSwift_PublishSubject()).refCount()
            default: return self.multicast(_RXSwift_ReplaySubject.create(bufferSize: replay)).refCount()
            }
        case .whileConnected:
            switch replay {
            case 0: return ShareWhileConnected(source: self.asObservable())
            case 1: return ShareReplay1WhileConnected(source: self.asObservable())
            default: return self.multicast(makeSubject: { _RXSwift_ReplaySubject.create(bufferSize: replay) }).refCount()
            }
        }
    }
}
...
private final class ShareReplay1WhileConnectedConnection<Element>
    ...
    init(parent: Parent, lock: _RXPlatform_RecursiveLock) {
        self._parent = parent
        self._lock = lock
    ...
    final private func _synchronized_on(_ event: _RXSwift_Event<Element>) -> Observers {
        if self._disposed {
            return Observers()
        }
    ...
    final func _synchronized_subscribe<Observer: _RXSwift_ObserverType>(_ observer: Observer) -> _RXSwift_Disposable where Observer.Element == Element {
        ...
        return _RXSwift_SubscriptionDisposable(owner: self, key: disposeKey)
    }
...
final private class ShareReplay1WhileConnected<Element>
    ...
    @inline(__always)
    private func _synchronized_subscribe<Observer: _RXSwift_ObserverType>(_ observer: Observer) -> Connection where Observer.Element == Element {
