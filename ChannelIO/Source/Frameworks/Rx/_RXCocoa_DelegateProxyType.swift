...
*/
protocol _RXCocoa_DelegateProxyType: class {
...
extension _RXCocoa_DelegateProxyType {
    /// Unique identifier for delegate
...
extension _RXCocoa_DelegateProxyType {
    ...
    static func _setCurrentDelegate(_ delegate: AnyObject?, to object: ParentObject) {
        return setCurrentDelegate(_RXCocoa_castOptionalOrFatalError(delegate), to: object)
    }
    ...
    func _setForwardToDelegate(_ forwardToDelegate: AnyObject?, retainDelegate: Bool) {
        return self.setForwardToDelegate(_RXCocoa_castOptionalOrFatalError(forwardToDelegate), retainDelegate: retainDelegate)
    }
}
...
extension _RXCocoa_DelegateProxyType {
    ...
    static func createProxy(for object: AnyObject) -> Self {
        return _RXCocoa_castOrFatalError(factory.createProxy(for: object))
    }
    ...
    static func proxy(for object: ParentObject) -> Self {
        ...
        if let existingProxy = maybeProxy {
            ...
            proxy = _RXCocoa_castOrFatalError(self.createProxy(for: object))
            self.assignProxy(proxy, toObject: object)
            assert(self.assignedProxy(for: object) === proxy)
        }
        let currentDelegate = self._currentDelegate(for: object)
        let delegateProxy: Self = _RXCocoa_castOrFatalError(proxy)
    ...
    static func installForwardDelegate(_ forwardDelegate: Delegate, retainDelegate: Bool, onProxyForObject object: ParentObject) -> _RXSwift_Disposable {
...
extension _RXCocoa_DelegateProxyType {
    ...
    private static func assignedProxy(for object: ParentObject) -> AnyObject? {
        ...
        return _RXCocoa_castOptionalOrFatalError(maybeDelegate)
    }
...
protocol _RXCocoa_HasDelegate: AnyObject {
    /// Delegate type
    associatedtype Delegate
...
extension _RXCocoa_DelegateProxyType where ParentObject: _RXCocoa_HasDelegate, Self.Delegate == ParentObject.Delegate {
    static func currentDelegate(for object: ParentObject) -> Delegate? {
...
protocol _RXCocoa_HasDataSource: AnyObject {
    /// Data source type
...
extension _RXCocoa_DelegateProxyType where ParentObject: _RXCocoa_HasDataSource, Self.Delegate == ParentObject.DataSource {
    static func currentDelegate(for object: ParentObject) -> Delegate? {
...
protocol _RXCocoa_HasPrefetchDataSource: AnyObject {
...
@available(iOS 10.0, tvOS 10.0, *)
extension _RXCocoa_DelegateProxyType where ParentObject: _RXCocoa_HasPrefetchDataSource, Self.Delegate == ParentObject.PrefetchDataSource {
    static func currentDelegate(for object: ParentObject) -> Delegate? {
    ...
    static func setCurrentDelegate(_ delegate: Delegate?, to object: ParentObject) {
        ...
        extension _RXSwift_ObservableType {
            func subscribeProxyDataSource<DelegateProxy: _RXCocoa_DelegateProxyType>(ofObject object: DelegateProxy.ParentObject, dataSource: DelegateProxy.Delegate, retainDataSource: Bool, binding: @escaping (DelegateProxy, _RXSwift_Event<Element>) -> Void)
                -> _RXSwift_Disposable
                ...
                let subscription = self.asObservable()
                    .observeOn(_RXSwift_MainScheduler())
                    .catchError { error in
                        _RXCocoa_bindingError(error)
                        return _RXSwift_Observable.empty()
                    }
                    // source can never end, otherwise it would release the subscriber, and deallocate the data source
                    .concat(_RXSwift_Observable.never())
                    .takeUntil(object.rx.deallocated)
                    .subscribe { [weak object] (event: _RXSwift_Event<Element>) in
                ...
                return _RXSwift_Disposables.create { [weak object] in
    ...
    private class DelegateProxyFactory {
        ...
        fileprivate static func sharedFactory<DelegateProxy: _RXCocoa_DelegateProxyType>(for proxyType: DelegateProxy.Type) -> DelegateProxyFactory {
            _RXSwift_MainScheduler.ensureRunningOnMainThread()
        ...
        private init<DelegateProxy: _RXCocoa_DelegateProxyType>(for proxyType: DelegateProxy.Type) {
            self._factories = [:]
            self._delegateProxyType = proxyType
            self._identifier = proxyType.identifier
        }

        fileprivate func extend<DelegateProxy: _RXCocoa_DelegateProxyType, ParentObject>(make: @escaping (ParentObject) -> DelegateProxy) {
                _RXSwift_MainScheduler.ensureRunningOnMainThread()
                ...
                guard self._factories[ObjectIdentifier(ParentObject.self)] == nil else {
                    _RXCocoa_rxFatalError("The factory of \(ParentObject.self) is duplicated. DelegateProxy is not allowed of duplicated base object type.")
                }
                self._factories[ObjectIdentifier(ParentObject.self)] = { make(_RXCocoa_castOrFatalError($0)) }
        }
        ...
        fileprivate func createProxy(for object: AnyObject) -> AnyObject {
            ...
            _RXCocoa_rxFatalError("DelegateProxy has no factory of \(object). Implement DelegateProxy subclass for \(object) first.")
        }
    }
