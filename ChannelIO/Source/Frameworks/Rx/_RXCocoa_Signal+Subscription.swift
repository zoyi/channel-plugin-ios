...
extension _RXSwift_SharedSequenceConvertibleType where SharingStrategy == _RXCocoa_SignalSharingStrategy {
     ...
     */
    func emit<Observer: _RXSwift_ObserverType>(to observer: Observer) -> _RXSwift_Disposable where Observer.Element == Element {
     ...
     */
    func emit(to relay: _RXRelay_BehaviorRelay<Element>) -> _RXSwift_Disposable {
    ...
    /**
     ...
     - returns: Disposable object that can be used to unsubscribe the observer from the relay.
     */
    func emit(to relay: _RXRelay_PublishRelay<Element>) -> _RXSwift_Disposable {
        return self.emit(onNext: { e in
            relay.accept(e)
        })
    }
     ...
     */
    func emit(onNext: ((Element) -> Void)? = nil, onCompleted: (() -> Void)? = nil, onDisposed: (() -> Void)? = nil) -> _RXSwift_Disposable {
