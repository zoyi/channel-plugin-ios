...
class AbstractDiffCalculator<Section: Equatable, Value: Equatable> {
    ...
    final func value(forSection: Int) -> Section {
    ...
    final func numberOfObjects(inSection section: Int) -> Int {
    ...
    final func value(atIndexPath indexPath: IndexPath) -> Value {
    ...
    final var sectionedValues: SectionedValues<Section, Value> {
        get {
...
final class TableViewDiffCalculator<Section: Equatable, Value: Equatable>: AbstractDiffCalculator<Section, Value> {
    ...
    init(tableView: UITableView?, initialSectionedValues: SectionedValues<Section, Value> = SectionedValues()) {
  ...
  var insertionAnimation = UITableView.RowAnimation.automatic, deletionAnimation = UITableView.RowAnimation.automatic

    var forceOffAnimationEnabled = false
...
final class CollectionViewDiffCalculator<Section: Equatable, Value: Equatable> : AbstractDiffCalculator<Section, Value> {
    ...
    ///   - initialSectionedValues: optional - if specified, these will be the initial contents of the diff calculator.
    init(collectionView: UICollectionView?, initialSectionedValues: SectionedValues<Section, Value> = SectionedValues()) {
...
final class SingleSectionTableViewDiffCalculator<Value: Equatable> {
    ...
    /// You can change insertion/deletion animations like this! Fade works well.
    /// So does Top/Bottom. Left/Right/Middle are a little weird, but hey, do your thing.
  var insertionAnimation = UITableView.RowAnimation.automatic {
        didSet {
            self.internalDiffCalculator.insertionAnimation = self.insertionAnimation 
        }
    }
    
  var deletionAnimation = UITableView.RowAnimation.automatic {
        didSet {
    ...
    init(tableView: UITableView?, initialRows: [Value] = [], sectionIndex: Int = 0) {
        self.tableView = tableView
...
final class SingleSectionCollectionViewDiffCalculator<Value: Equatable> {
    ...
    ///   - sectionIndex: optional - all insertion/deletion calls will be made on this index.
    init(collectionView: UICollectionView?, initialItems: [Value] = [], sectionIndex: Int = 0) {
        self.collectionView = collectionView
