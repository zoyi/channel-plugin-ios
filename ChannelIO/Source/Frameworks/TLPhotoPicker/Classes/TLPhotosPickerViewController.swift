...
extension TLPhotosPickerViewControllerDelegate {
    ...
    func dismissPhotoPicker(withPHAssets: [PHAsset]) { }
    func dismissPhotoPicker(withTLPHAssets: [TLPHAsset]) { }
    func shouldDismissPhotoPicker(withTLPHAssets: [TLPHAsset]) -> Bool { return true }
    func dismissComplete() { }
    func photoPickerDidCancel() { }
...
struct TLPhotosPickerConfigure {
    ...
    var tapHereToChange = "Tap here to change"
    var cancelTitle = "Cancel"
    var doneTitle = "Done"
    var emptyMessage = "No albums"
    var selectMessage = "Select"
    var deselectMessage = "Deselect"
    var emptyImage: UIImage? = nil
    var usedCameraButton = true
    var usedPrefetch = false
    var previewAtForceTouch = false
    var startplayBack: PHLivePhotoViewPlaybackStyle = .hint
    var allowedLivePhotos = true
    var allowedVideo = true
    var allowedAlbumCloudShared = false
    var allowedPhotograph = true
    var allowedVideoRecording = true
    var recordingVideoQuality: UIImagePickerController.QualityType = .typeMedium
    ...
    var fetchCollectionOption: [TL_FetchCollectionType: PHFetchOptions] = [:]
    var selectedColor = UIColor(red: 88/255, green: 144/255, blue: 255/255, alpha: 1.0)
    var cameraBgColor = UIColor(red: 221/255, green: 223/255, blue: 226/255, alpha: 1)
    var cameraIcon = TLBundle.podBundleImage(named: "TLPhotoPicker_camera")
    var videoIcon = TLBundle.podBundleImage(named: "TLPhotoPicker_video")
    var placeholderIcon = TLBundle.podBundleImage(named: "TLPhotoPicker_insertPhotoMaterial")
    var nibSet: (nibName: String, bundle:Bundle)? = nil
    var cameraCellNibSet: (nibName: String, bundle:Bundle)? = nil
    var fetchCollectionTypes: [(PHAssetCollectionType,PHAssetCollectionSubtype)]? = nil
    var groupByFetch: TL_PHFetchedResultGroupedBy? = nil
    var supportedInterfaceOrientations: UIInterfaceOrientationMask = .portrait
    var popup: [TL_PopupConfigure] = []
    init() {
        
    }
}

enum TL_FetchCollectionType {
    case assetCollections(PHAssetCollectionType)
    case topLevelUserCollections
}

extension TL_FetchCollectionType: Hashable {
...
class TLPhotosPickerViewController: UIViewController {
    ...
    @IBOutlet var customNavItem: UINavigationItem!
    ...
    weak var delegate: TLPhotosPickerViewControllerDelegate? = nil
    weak var logDelegate: TLPhotosPickerLogDelegate? = nil
    var selectedAssets = [TLPHAsset]()
    var configure = TLPhotosPickerConfigure()
    var customDataSouces: TLPhotopickerDataSourcesProtocol? = nil
    ...
    @objc var canSelectAsset: ((PHAsset) -> Bool)? = nil
    @objc var didExceedMaximumNumberOfSelection: ((TLPhotosPickerViewController) -> Void)? = nil
    @objc var handleNoAlbumPermissions: ((TLPhotosPickerViewController) -> Void)? = nil
    @objc var handleNoCameraPermissions: ((TLPhotosPickerViewController) -> Void)? = nil
    @objc var dismissCompletion: (() -> Void)? = nil
    ...
    private var requestIDs = TL_SynchronizedDictionary<IndexPath,PHImageRequestID>()
    ...
    @objc convenience init(withPHAssets: (([PHAsset]) -> Void)? = nil, didCancel: (() -> Void)? = nil) {
        self.init()
        self.completionWithPHAssets = withPHAssets
        self.didCancel = didCancel
    }
    
    convenience init(withTLPHAssets: (([TLPHAsset]) -> Void)? = nil, didCancel: (() -> Void)? = nil) {
        self.init()
    ...
    override func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        ...
        if #available(iOS 13.0, *) {
            ...
            let image = TLBundle.podBundleImage(named: "TLPhotoPicker_pop_arrow")
    ...
    func deselectWhenUsingSingleSelectedMode() {
        if
...
extension TLPhotosPickerViewController {
    ...
    @objc func registerNib(nibName: String, bundle: Bundle) {
    ...
    @objc func makeUI() {
        ...
        self.popArrowImageView.image = TLBundle.podBundleImage(named: "TLPhotoPicker_pop_arrow")
        self.subTitleArrowImageView.image = TLBundle.podBundleImage(named: "TLPhotoPicker_arrow")
...
extension Array where Element == TL_PopupConfigure {
