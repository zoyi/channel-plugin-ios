 ...
 */
final class PhoneNumberKit_ParseManager {
    weak var metadataManager: PhoneNumberKit_MetadataManager?
    let parser: PhoneNumberKit_PhoneNumberParser
    weak var regexManager: PhoneNumberKit_RegexManager?

    init(metadataManager: PhoneNumberKit_MetadataManager, regexManager: PhoneNumberKit_RegexManager) {
        self.metadataManager = metadataManager
        self.parser = PhoneNumberKit_PhoneNumberParser(regex: regexManager, metadata: metadataManager)
     ...
     */
    func parse(_ numberString: String, withRegion region: String, ignoreType: Bool) throws -> PhoneNumberKit_PhoneNumber {
        guard let metadataManager = metadataManager, let regexManager = regexManager else { throw PhoneNumberKit_PhoneNumberError.generalError }
        ...
        nationalNumber = regexManager.stringByReplacingOccurrences(matchedNumber, map: PhoneNumberKit_PhoneNumberPatterns.allNormalizationMappings, keepUnmapped: true)
        ...
        guard var regionMetadata = metadataManager.territoriesByCountry[region] else {
            throw PhoneNumberKit_PhoneNumberError.invalidCountryCode
        }
        ...
        do {
            ...
            let plusRemovedNumberString = regexManager.replaceStringByRegex(PhoneNumberKit_PhoneNumberPatterns.leadingPlusCharsPattern, string: nationalNumber as String)
        ...
        if let generalNumberDesc = regionMetadata.generalDesc, regexManager.hasValue(generalNumberDesc.nationalNumberPattern) == false || parser.isNumberMatchingDesc(nationalNumber, numberDesc: generalNumberDesc) == false {
            throw PhoneNumberKit_PhoneNumberError.notANumber
        }
        ...
        var type: PhoneNumberKit_PhoneNumberType = .unknown
        if ignoreType == false {
            ...
            if type == .unknown {
                throw PhoneNumberKit_PhoneNumberError.unknownType
            }
        }

        let phoneNumber = PhoneNumberKit_PhoneNumber(numberString: numberString, countryCode: countryCode, leadingZero: leadingZero, nationalNumber: finalNationalNumber, numberExtension: numberExtension, type: type, regionID: regionMetadata.codeID)
        return phoneNumber
    }
     ...
     */
    func parseMultiple(_ numberStrings: [String], withRegion region: String, ignoreType: Bool, shouldReturnFailedEmptyNumbers: Bool = false, testCallback: (() -> Void)? = nil) -> [PhoneNumberKit_PhoneNumber] {
        var multiParseArray = [PhoneNumberKit_PhoneNumber]()
            ...
            queue.async(group: group) {
                ...
                do {
                    if let phoneNumber = try self?.parse(numberString, withRegion: region, ignoreType: ignoreType) {
                        ...
                        multiParseArray.append(PhoneNumberKit_PhoneNumber.notPhoneNumber())
                    }
                } catch {
