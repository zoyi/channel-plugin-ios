...
/// Partial formatter
final class PhoneNumberKit_PartialFormatter {
    private let phoneNumberKit: PhoneNumberKit

    weak var metadataManager: PhoneNumberKit_MetadataManager?
    weak var parser: PhoneNumberKit_PhoneNumberParser?
    weak var regexManager: PhoneNumberKit_RegexManager?

    convenience init(phoneNumberKit: PhoneNumberKit = PhoneNumberKit(), defaultRegion: String = PhoneNumberKit.defaultRegionCode(), withPrefix: Bool = true, maxDigits: Int? = nil) {
    ...
    init(phoneNumberKit: PhoneNumberKit, regexManager: PhoneNumberKit_RegexManager, metadataManager: PhoneNumberKit_MetadataManager, parser: PhoneNumberKit_PhoneNumberParser, defaultRegion: String, withPrefix: Bool = true, maxDigits: Int? = nil) {
    ...
    var defaultMetadata: PhoneNumberKit_MetadataTerritory?
    var currentMetadata: PhoneNumberKit_MetadataTerritory?
    ...
    func nationalNumber(from rawNumber: String) -> String {
    ...
    func formatPartial(_ rawNumber: String) -> String {
        ...
        if self.withPrefix, self.shouldAddSpaceAfterNationalPrefix, self.prefixBeforeNationalNumber.count > 0, self.prefixBeforeNationalNumber.last != PhoneNumberKit_PhoneNumberConstants.separatorBeforeNationalNumber.first {
            finalNumber.append(PhoneNumberKit_PhoneNumberConstants.separatorBeforeNationalNumber)
        }
        if nationalNumber.count > 0 {
            finalNumber.append(nationalNumber)
        }
        if finalNumber.last == PhoneNumberKit_PhoneNumberConstants.separatorBeforeNationalNumber.first {
    ...
    internal func isValidRawNumber(_ rawNumber: String) -> Bool {
        do {
            ...
            let validPartialPattern = "[+＋]?(\\s*\\d)+\\s*$|\(PhoneNumberKit_PhoneNumberPatterns.validPhoneNumberPattern)"
            let validNumberMatches = try regexManager?.regexMatches(validPartialPattern, string: rawNumber)
            let validStart = self.regexManager?.stringPositionByRegex(PhoneNumberKit_PhoneNumberPatterns.validStartPattern, string: rawNumber)
    ...
    func isFormatEligible(_ format: PhoneNumberKit_MetadataPhoneNumberFormat) -> Bool {
        guard let phoneFormat = format.format else {
            return false
        }
        do {
            let validRegex = try regexManager?.regexWithPattern(PhoneNumberKit_PhoneNumberPatterns.eligibleAsYouTypePattern)
    ...
    func extractIDD(_ rawNumber: String) -> String {
        ...
        do {
            if let internationalPrefix = currentMetadata?.internationalPrefix {
                let prefixPattern = String(format: PhoneNumberKit_PhoneNumberPatterns.iddPattern, arguments: [internationalPrefix])
        ...
        if self.isNanpaNumberWithNationalPrefix(rawNumber) {
            ...
            do {
                if let nationalPrefix = currentMetadata?.nationalPrefixForParsing {
                    let nationalPrefixPattern = String(format: PhoneNumberKit_PhoneNumberPatterns.nationalPrefixParsingPattern, arguments: [nationalPrefix])
    ...
    func extractCountryCallingCode(_ rawNumber: String) -> String {
        ...
        if self.prefixBeforeNationalNumber.isEmpty == false, self.prefixBeforeNationalNumber.first != "+" {
            self.prefixBeforeNationalNumber.append(PhoneNumberKit_PhoneNumberConstants.separatorBeforeNationalNumber)
        }
    ...
    func availableFormats(_ rawNumber: String) -> [PhoneNumberKit_MetadataPhoneNumberFormat]? {
        guard let regexManager = regexManager else { return nil }
        var tempPossibleFormats = [PhoneNumberKit_MetadataPhoneNumberFormat]()
        var possibleFormats = [PhoneNumberKit_MetadataPhoneNumberFormat]()
    ...
    func applyFormat(_ rawNumber: String, formats: [PhoneNumberKit_MetadataPhoneNumberFormat]) -> String? {
        guard let regexManager = regexManager else { return nil }
        for format in formats {
            if let pattern = format.pattern, let formatTemplate = format.format {
                let patternRegExp = String(format: PhoneNumberKit_PhoneNumberPatterns.formatPattern, arguments: [pattern])
                do {
                    ...
                    if matches.count > 0 {
                        if let nationalPrefixFormattingRule = format.nationalPrefixFormattingRule {
                            let separatorRegex = try regexManager.regexWithPattern(PhoneNumberKit_PhoneNumberPatterns.prefixSeparatorPattern)
    ...
    func createFormattingTemplate(_ format: PhoneNumberKit_MetadataPhoneNumberFormat, rawNumber: String) -> String? {
        ...
        do {
            let characterClassRegex = try regexManager.regexWithPattern(PhoneNumberKit_PhoneNumberPatterns.characterClassPattern)
            numberPattern = characterClassRegex.stringByReplacingMatches(in: numberPattern, withTemplate: "\\\\d")

            let standaloneDigitRegex = try regexManager.regexWithPattern(PhoneNumberKit_PhoneNumberPatterns.standaloneDigitPattern)
    ...
    func getFormattingTemplate(_ numberPattern: String, numberFormat: String, rawNumber: String) -> String? {
        guard let regexManager = regexManager else { return nil }
        do {
            let matches = try regexManager.matchedStringByRegex(numberPattern, string: PhoneNumberKit_PhoneNumberConstants.longPhoneNumber)
            if let match = matches.first {
                ...
                template = regexManager.replaceStringByRegex("9", string: template, template: PhoneNumberKit_PhoneNumberConstants.digitPlaceholder)
                return template
            }
        } catch {}
        return nil
    ...
    func applyFormattingTemplate(_ template: String, rawNumber: String) -> String {
        ...
        for character in template {
            if character == PhoneNumberKit_PhoneNumberConstants.digitPlaceholder.first {
