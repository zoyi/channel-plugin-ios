...
import CoreTelephony
 ...
 typealias PhoneNumberKit_MetadataCallback = (() throws -> Data?)

 final class PhoneNumberKit: NSObject {
    // Manager objects
    let metadataManager: PhoneNumberKit_MetadataManager
    let parseManager: PhoneNumberKit_ParseManager
    let regexManager = PhoneNumberKit_RegexManager()

    // MARK: Lifecycle

     init(metadataCallback: @escaping PhoneNumberKit_MetadataCallback = PhoneNumberKit.defaultMetadataCallback) {
        self.metadataManager = PhoneNumberKit_MetadataManager(metadataCallback: metadataCallback)
        self.parseManager = PhoneNumberKit_ParseManager(metadataManager: self.metadataManager, regexManager: self.regexManager)
    }
    ...
    /// - Returns: PhoneNumber object.
     func parse(_ numberString: String, withRegion region: String = PhoneNumberKit.defaultRegionCode(), ignoreType: Bool = false) throws -> PhoneNumberKit_PhoneNumber {
    ...
    /// - returns: array of PhoneNumber objects.
     func parse(_ numberStrings: [String], withRegion region: String = PhoneNumberKit.defaultRegionCode(), ignoreType: Bool = false, shouldReturnFailedEmptyNumbers: Bool = false) -> [PhoneNumberKit_PhoneNumber] {
    ...
    /// - returns: Formatted representation of the PhoneNumber.
     func format(_ phoneNumber: PhoneNumberKit_PhoneNumber, toType formatType: PhoneNumberKit_PhoneNumberFormat, withPrefix prefix: Bool = true) -> String {
        if formatType == .e164 {
            ...
            let formatter = PhoneNumberKit_Formatter(phoneNumberKit: self)
    ...
    /// - returns: optional array of ISO 639 compliant region codes.
     func countries(withCode countryCode: UInt64) -> [String]? {
    ...
    /// - returns: ISO 639 compliant region code string.
     func mainCountry(forCode countryCode: UInt64) -> String? {
    ...
    /// - returns: leading digits (e.g. 876 for Jamaica).
     func leadingDigits(for country: String) -> String? {
    ...
    /// - returns: Region code, eg "US", or nil if the region cannot be determined.
     func getRegionCode(of phoneNumber: PhoneNumberKit_PhoneNumber) -> String? {
    ...
    /// - returns: An example phone number
     func getExampleNumber(forCountry countryCode: String, ofType type: PhoneNumberKit_PhoneNumberType = .mobile) -> PhoneNumberKit_PhoneNumber? {
    ...
    /// - returns: A formatted example phone number
     func getFormattedExampleNumber(
        forCountry countryCode: String, ofType type: PhoneNumberKit_PhoneNumberType = .mobile,
        withFormat format: PhoneNumberKit_PhoneNumberFormat = .international, withPrefix prefix: Bool = true
    ) -> String? {
    ...
    /// - returns: A MetadataTerritory object, or nil if no metadata was found for the country code
     func metadata(for country: String) -> PhoneNumberKit_MetadataTerritory? {
    ...
    ///
    /// - parameter countryCode: international country code (e.g 44 for the UK)
     func metadata(forCode countryCode: UInt64) -> [PhoneNumberKit_MetadataTerritory]? {
    ...
    /// - returns: Array of possible lengths for the country. May be empty.
     func possiblePhoneNumberLengths(forCountry country: String, phoneNumberType: PhoneNumberKit_PhoneNumberType, lengthType: PhoneNumberKit_PossibleLengthType) -> [Int] {
    ...
    private func possiblePhoneNumberLengths(forTerritory territory: PhoneNumberKit_MetadataTerritory, phoneNumberType: PhoneNumberKit_PhoneNumberType) -> PhoneNumberKit_MetadataPossibleLengths? {
    ...
    /// - returns: A computed value for the user's current region - based on the iPhone's carrier and if not available, the device region.
     class func defaultRegionCode() -> String {
        ...
        return PhoneNumberKit_PhoneNumberConstants.defaultCountry
    }
    ...
    /// - returns: an optional Data representation of the metadata.
     static func defaultMetadataCallback() throws -> Data? {
      guard let jsonPath = CHAssets.getBundle().path(forResource: "PhoneNumberMetadata", ofType: "json") else {
            throw PhoneNumberKit_PhoneNumberError.metadataNotFound
        }
    ...
    /// Configuration for the CountryCodePicker presented from PhoneNumberTextField if `withDefaultPickerUI` is `true`
     enum CountryCodePicker {
        /// Common Country Codes are shown below the Current section in the CountryCodePicker by default
         static var commonCountryCodes: [String] = []

        /// When the Picker is shown from the textfield it is presented modally
         static var forceModalPresentation: Bool = false
    }
}
#endif
