...
final class PhoneNumberKit_MetadataManager {
    var territories = [PhoneNumberKit_MetadataTerritory]()
    var territoriesByCode = [UInt64: [PhoneNumberKit_MetadataTerritory]]()
    var mainTerritoryByCode = [UInt64: PhoneNumberKit_MetadataTerritory]()
    var territoriesByCountry = [String: PhoneNumberKit_MetadataTerritory]()
    ...
    init(metadataCallback: PhoneNumberKit_MetadataCallback) {
        self.territories = self.populateTerritories(metadataCallback: metadataCallback)
        for item in self.territories {
            var currentTerritories: [PhoneNumberKit_MetadataTerritory] = self.territoriesByCode[item.countryCode] ?? [PhoneNumberKit_MetadataTerritory]()
    ...
    private func populateTerritories(metadataCallback: PhoneNumberKit_MetadataCallback) -> [PhoneNumberKit_MetadataTerritory] {
        var territoryArray = [PhoneNumberKit_MetadataTerritory]()
        do {
            let jsonData: Data? = try metadataCallback()
            let jsonDecoder = JSONDecoder()
            if let jsonData = jsonData, let metadata: PhoneNumberKit_PhoneNumberMetadata = try? jsonDecoder.decode(PhoneNumberKit_PhoneNumberMetadata.self, from: jsonData) {
    ...
    internal func filterTerritories(byCode code: UInt64) -> [PhoneNumberKit_MetadataTerritory]? {
    ...
    internal func filterTerritories(byCountry country: String) -> PhoneNumberKit_MetadataTerritory? {
    ...
    ///
    /// - returns: A MetadataTerritory object.
    internal func mainTerritory(forCode code: UInt64) -> PhoneNumberKit_MetadataTerritory? {
