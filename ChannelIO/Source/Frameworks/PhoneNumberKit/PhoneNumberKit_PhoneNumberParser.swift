 ...
 */
final class PhoneNumberKit_PhoneNumberParser {
    let metadata: PhoneNumberKit_MetadataManager
    let regex: PhoneNumberKit_RegexManager

    init(regex: PhoneNumberKit_RegexManager, metadata: PhoneNumberKit_MetadataManager) {
        self.regex = regex
    ...
    func normalizePhoneNumber(_ number: String) -> String {
        let normalizationMappings = PhoneNumberKit_PhoneNumberPatterns.allNormalizationMappings
     ...
     */
    func extractCountryCode(_ number: String, nationalNumber: inout String, metadata: PhoneNumberKit_MetadataTerritory) throws -> UInt64 {
        ...
        if countryCodeSource != .defaultCountry {
            if fullNumber.count <= PhoneNumberKit_PhoneNumberConstants.minLengthForNSN {
                throw PhoneNumberKit_PhoneNumberError.tooShort
            }
    ...
    func extractPotentialCountryCode(_ fullNumber: String, nationalNumber: inout String) -> UInt64? {
        ...
        let maxCountryCode = PhoneNumberKit_PhoneNumberConstants.maxLengthCountryCode
        var startPosition = 0
    ...
    func checkNumberType(_ nationalNumber: String, metadata: PhoneNumberKit_MetadataTerritory, leadingZero: Bool = false) -> PhoneNumberKit_PhoneNumberType {
        if leadingZero {
     ...
     */
    func isNumberMatchingDesc(_ nationalNumber: String, numberDesc: PhoneNumberKit_MetadataPhoneNumberDesc?) -> Bool {
    ...
    func parsePrefixAsIdd(_ number: inout String, iddPattern: String) -> Bool {
        if self.regex.stringPositionByRegex(iddPattern, string: number) == 0 {
            do {
                ...
                let capturingDigitPatterns = try NSRegularExpression(pattern: PhoneNumberKit_PhoneNumberPatterns.capturingDigitPattern, options: NSRegularExpression.Options.caseInsensitive)
                ...
                if let firstMatch = matchedGroups.first {
                    ...
                    if digitMatched.length > 0 {
                        let normalizedGroup = self.regex.stringByReplacingOccurrences(digitMatched as String, map: PhoneNumberKit_PhoneNumberPatterns.allNormalizationMappings)
    ...
    func stripExtension(_ number: inout String) -> String? {
        do {
            let matches = try regex.regexMatches(PhoneNumberKit_PhoneNumberPatterns.extnPattern, string: number)
     ...
     */
    func stripInternationalPrefixAndNormalize(_ number: inout String, possibleIddPrefix: String?) -> PhoneNumberKit_PhoneNumberCountryCodeSource {
        if self.regex.matchesAtStart(PhoneNumberKit_PhoneNumberPatterns.leadingPlusCharsPattern, string: number as String) {
            number = self.regex.replaceStringByRegex(PhoneNumberKit_PhoneNumberPatterns.leadingPlusCharsPattern, string: number as String)
     ...
     */
    func stripNationalPrefix(_ number: inout String, metadata: PhoneNumberKit_MetadataTerritory) {
