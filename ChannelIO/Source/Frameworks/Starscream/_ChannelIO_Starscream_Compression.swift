...
class Starscream_Decompressor {
    ...
    func reset() throws {
        teardownInflate()
        guard initInflate() else { throw Starscream_WSError(type: .compressionError, message: "Error for decompressor on reset", code: 0) }
    }
    ...
    private func decompress(bytes: UnsafePointer<UInt8>, count: Int, out:inout Data) throws {
        ...
        guard (res == Z_OK && strm.avail_out > 0)
            || (res == Z_BUF_ERROR && Int(strm.avail_out) == buffer.count)
                ...
                throw Starscream_WSError(type: .compressionError, message: "Error on decompressing", code: 0)
        }
    }
...
class Starscream_Compressor {
    ...
    func reset() throws {
        teardownDeflate()
        guard initDeflate() else { throw Starscream_WSError(type: .compressionError, message: "Error for compressor on reset", code: 0) }
    }
    ...
    func compress(_ data: Data) throws -> Data {
        ...
        guard res == Z_OK && strm.avail_out > 0
            ...
            throw Starscream_WSError(type: .compressionError, message: "Error on compressing", code: 0)
        }
