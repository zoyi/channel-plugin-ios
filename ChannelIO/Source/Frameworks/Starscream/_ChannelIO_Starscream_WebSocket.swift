...
let Starscream_WebsocketDidConnectNotification = "WebsocketDidConnectNotification"
let Starscream_WebsocketDidDisconnectNotification = "WebsocketDidDisconnectNotification"
let Starscream_WebsocketDisconnectionErrorKeyName = "WebsocketDisconnectionErrorKeyName"

//Standard WebSocket close codes
enum Starscream_CloseCode : UInt16 {
    case normal                 = 1000
    case goingAway              = 1001
...
enum Starscream_ErrorType: Error {
...
struct Starscream_WSError: Error {
    let type: Starscream_ErrorType
    let message: String
    let code: Int
}

//WebSocketClient is setup to be dependency injection for testing
protocol Starscream_WebSocketClient: class {
    var delegate: Starscream_WebSocketDelegate? {get set}
    var pongDelegate: Starscream_WebSocketPongDelegate? {get set}
    var disableSSLCertValidation: Bool {get set}
    var overrideTrustHostname: Bool {get set}
    var desiredTrustHostname: String? {get set}
    var sslClientCertificate: Starscream_SSLClientCertificate? {get set}
    #if os(Linux)
    #else
    var security: Starscream_SSLTrustValidator? {get set}
...
extension Starscream_WebSocketClient {
    ...
    func disconnect() {
        disconnect(forceTimeout: nil, closeCode: Starscream_CloseCode.normal.rawValue)
    }
}

//SSL settings for the stream
struct Starscream_SSLSettings {
    let useSSL: Bool
    let disableCertValidation: Bool
    var overrideTrustHostname: Bool
    var desiredTrustHostname: String?
    let sslClientCertificate: Starscream_SSLClientCertificate?
    #if os(Linux)
    #else
...
protocol Starscream_WSStreamDelegate: class {
...
protocol Starscream_WSStream {
    var delegate: Starscream_WSStreamDelegate? {get set}
    func connect(url: URL, port: Int, timeout: TimeInterval, ssl: Starscream_SSLSettings, completion: @escaping ((Error?) -> Void))
...
class Starscream_FoundationStream : NSObject, Starscream_WSStream, StreamDelegate  {
    private let workQueue = DispatchQueue(label: "com.vluxe.channelio.starscream.websocket", attributes: [])
    private var inputStream: InputStream?
    private var outputStream: OutputStream?
    weak var delegate: Starscream_WSStreamDelegate?
    let BUFFER_MAX = 4096
    ...
    func connect(url: URL, port: Int, timeout: TimeInterval, ssl: Starscream_SSLSettings, completion: @escaping ((Error?) -> Void)) {
            ...
            if let cipherSuites = ssl.cipherSuites {
                ...
                if let sslContextIn = CFReadStreamCopyProperty(inputStream, CFStreamPropertyKey(rawValue: kCFStreamPropertySSLContext)) as! SSLContext?,
                    ...
                    if resIn != errSecSuccess {
                        completion(Starscream_WSError(type: .invalidSSLError, message: "Error setting ingoing cypher suites", code: Int(resIn)))
                    }
                    if resOut != errSecSuccess {
                        completion(Starscream_WSError(type: .invalidSSLError, message: "Error setting outgoing cypher suites", code: Int(resOut)))
                    }
                }
                #endif
            }
        ...
        workQueue.async { [weak self] in
            while !outStream.hasSpaceAvailable {
                ...
                out -= 100
                if out < 0 {
                    completion(Starscream_WSError(type: .writeTimeoutError, message: "Timed out waiting for the socket to be ready for a write", code: 0))
                    return
                } else if let error = outStream.streamError {
                    completion(error)
                    return // disconnectStream will be called.
                } else if self == nil {
                    completion(Starscream_WSError(type: .closeError, message: "socket object has been dereferenced", code: 0))
                    return
                }
            }
...
protocol Starscream_WebSocketDelegate: class {
    func websocketDidConnect(socket: Starscream_WebSocketClient)
    func websocketDidDisconnect(socket: Starscream_WebSocketClient, error: Error?)
    func websocketDidReceiveMessage(socket: Starscream_WebSocketClient, text: String)
    func websocketDidReceiveData(socket: Starscream_WebSocketClient, data: Data)
}

//got pongs
protocol Starscream_WebSocketPongDelegate: class {
    func websocketDidReceivePong(socket: Starscream_WebSocketClient, data: Data?)
}

// A Delegate with more advanced info on messages and connection etc.
protocol Starscream_WebSocketAdvancedDelegate: class {
    func websocketDidConnect(socket: Starscream_WebSocket)
    func websocketDidDisconnect(socket: Starscream_WebSocket, error: Error?)
    func websocketDidReceiveMessage(socket: Starscream_WebSocket, text: String, response: Starscream_WebSocket.WSResponse)
    func websocketDidReceiveData(socket: Starscream_WebSocket, data: Data, response: Starscream_WebSocket.WSResponse)
    func websocketHttpUpgrade(socket: Starscream_WebSocket, request: String)
    func websocketHttpUpgrade(socket: Starscream_WebSocket, response: String)
}


class Starscream_WebSocket : NSObject, StreamDelegate, Starscream_WebSocketClient, Starscream_WSStreamDelegate {
    ...
    weak var delegate: Starscream_WebSocketDelegate?
    
    /// The optional advanced delegate can be used instead of of the delegate
    weak var advancedDelegate: Starscream_WebSocketAdvancedDelegate?

    /// Receives a callback for each pong message recived.
    weak var pongDelegate: Starscream_WebSocketPongDelegate?
    ...
    var sslClientCertificate: Starscream_SSLClientCertificate? = nil

    var enableCompression = true
    #if os(Linux)
    #else
    var security: Starscream_SSLTrustValidator?
    ...
    private struct CompressionState {
        ...
        var decompressor:Starscream_Decompressor? = nil
        var compressor:Starscream_Compressor? = nil
    }
    
    private var stream: Starscream_WSStream
    ...
    init(request: URLRequest, protocols: [String]? = nil, stream: Starscream_WSStream = Starscream_FoundationStream()) {
    ...
    */
    func disconnect(forceTimeout: TimeInterval? = nil, closeCode: UInt16 = Starscream_CloseCode.normal.rawValue) {
    ...
    private func initStreamsWithData(_ data: Data, _ port: Int) {
        ...
        #else
            let settings = Starscream_SSLSettings(useSSL: useSSL,
                                       disableCertValidation: disableSSLCertValidation,
                                       overrideTrustHostname: overrideTrustHostname,
                                       desiredTrustHostname: desiredTrustHostname,
                                       sslClientCertificate: sslClientCertificate,
                ...
                // Do the pinning now if needed
                    ...
                    if let sec = self.security, !self.certValidated {
                        ...
                        if !self.certValidated {
                            self.disconnectStream(Starscream_WSError(type: .invalidSSLError, message: "Invalid SSL certificate", code: 0))
                            return
                        }
                    }
                #endif
    ...
    private func processTCPHandshake(_ buffer: UnsafePointer<UInt8>, bufferLen: Int) {
        ...
        default:
            doDisconnect(Starscream_WSError(type: .upgradeError, message: "Invalid HTTP upgrade", code: code))
        }
    }
        ...
        if totalSize > 0 {
            ...
            if canDispatch {
                callbackQueue.async { [weak self] in
                    ...
                    NotificationCenter.default.post(name: NSNotification.Name(Starscream_WebsocketDidConnectNotification), object: self)
                }
            }
    ...
    func processExtensionHeader(_ extensionHeader: String) {
        ...
        if compressionState.supportsCompression {
            compressionState.decompressor = Starscream_Decompressor(windowBits: compressionState.serverMaxWindowBits)
            compressionState.compressor = Starscream_Compressor(windowBits: compressionState.clientMaxWindowBits)
        }
    }
    ...
    private func processOneRawMessage(inBuffer buffer: UnsafeBufferPointer<UInt8>) -> UnsafeBufferPointer<UInt8> {
        ...
        if let response = response, response.bytesLeft > 0 {
            ...
            if (isMasked > 0 || (RSVMask & baseAddress[0]) > 0) && receivedOpcode != .pong && !compressionState.messageNeedsDecompression {
                let errCode = Starscream_CloseCode.protocolError.rawValue
                doDisconnect(Starscream_WSError(type: .protocolError, message: "masked and rsv data is not currently supported", code: Int(errCode)))
                writeError(errCode)
                return emptyBuffer
            }
            ...
            if !isControlFrame && (receivedOpcode != .binaryFrame && receivedOpcode != .continueFrame &&
                receivedOpcode != .textFrame && receivedOpcode != .pong) {
                    let errCode = Starscream_CloseCode.protocolError.rawValue
                    doDisconnect(Starscream_WSError(type: .protocolError, message: "unknown opcode: \(receivedOpcodeRawValue)", code: Int(errCode)))
                    writeError(errCode)
                    return emptyBuffer
            }
            if isControlFrame && isFin == 0 {
                let errCode = Starscream_CloseCode.protocolError.rawValue
                doDisconnect(Starscream_WSError(type: .protocolError, message: "control frames can't be fragmented", code: Int(errCode)))
                writeError(errCode)
                return emptyBuffer
            }
            var closeCode = Starscream_CloseCode.normal.rawValue
            if receivedOpcode == .connectionClose {
                if payloadLen == 1 {
                    closeCode = Starscream_CloseCode.protocolError.rawValue
                } else if payloadLen > 1 {
                    closeCode = Starscream_WebSocket.readUint16(baseAddress, offset: offset)
                    if closeCode < 1000 || (closeCode > 1003 && closeCode < 1007) || (closeCode > 1013 && closeCode < 3000) {
                        closeCode = Starscream_CloseCode.protocolError.rawValue
                    }
                }
                if payloadLen < 2 {
                    doDisconnect(Starscream_WSError(type: .protocolError, message: "connection closed by server", code: Int(closeCode)))
                    writeError(closeCode)
                    return emptyBuffer
                }
            } else if isControlFrame && payloadLen > 125 {
                writeError(Starscream_CloseCode.protocolError.rawValue)
                return emptyBuffer
            }
            var dataLength = UInt64(payloadLen)
            if dataLength == 127 {
                dataLength = Starscream_WebSocket.readUint64(baseAddress, offset: offset)
                offset += MemoryLayout<UInt64>.size
            } else if dataLength == 126 {
                dataLength = UInt64(Starscream_WebSocket.readUint16(baseAddress, offset: offset))
            ...
            if compressionState.messageNeedsDecompression, let decompressor = compressionState.decompressor {
                do {
                    ...
                    let closeCode = Starscream_CloseCode.encoding.rawValue
                    doDisconnect(Starscream_WSError(type: .protocolError, message: closeReason, code: Int(closeCode)))
                    writeError(closeCode)
                    return emptyBuffer
                }
            } else {
            ...
            if isFin == 0 && receivedOpcode == .continueFrame && response == nil {
                let errCode = Starscream_CloseCode.protocolError.rawValue
                doDisconnect(Starscream_WSError(type: .protocolError, message: "continue frame before a binary or text frame", code: Int(errCode)))
                writeError(errCode)
                return emptyBuffer
            }
            var isNew = false
            if response == nil {
                if receivedOpcode == .continueFrame {
                    let errCode = Starscream_CloseCode.protocolError.rawValue
                    doDisconnect(Starscream_WSError(type: .protocolError, message: "first frame can't be a continue frame", code: Int(errCode)))
                    writeError(errCode)
                    return emptyBuffer
                }
                isNew = true
                ...
                if receivedOpcode == .continueFrame {
                    ...
                    let errCode = Starscream_CloseCode.protocolError.rawValue
                    doDisconnect(Starscream_WSError(type: .protocolError, message: "second and beyond of fragment message must be a continue frame", code: Int(errCode)))
                    writeError(errCode)
                    return emptyBuffer
                }
        ...
        if response.isFin && response.bytesLeft <= 0 {
            if response.code == .ping {
                ...
                guard let str = String(data: response.buffer! as Data, encoding: .utf8) else {
                    writeError(Starscream_CloseCode.encoding.rawValue)
                    return false
                }
                if canDispatch {
                    callbackQueue.async { [weak self] in
    ...
    private func writeError(_ code: UInt16) {
        ...
        let buffer = UnsafeMutableRawPointer(mutating: buf!.bytes).assumingMemoryBound(to: UInt8.self)
        Starscream_WebSocket.writeUint16(buffer, offset: 0, value: code)
    ...
    private func dequeueWrite(_ data: Data, code: OpCode, writeCompletion: (() -> ())? = nil) {
        ...
        operation.addExecutionBlock { [weak self, weak operation] in
            ...
            if dataLength < 126 {
                ...
                buffer[1] = 126
                Starscream_WebSocket.writeUint16(buffer, offset: offset, value: UInt16(dataLength))
                offset += MemoryLayout<UInt16>.size
            } else {
                buffer[1] = 127
                Starscream_WebSocket.writeUint64(buffer, offset: offset, value: UInt64(dataLength))
                offset += MemoryLayout<UInt64>.size
            }
            ...
            while !sOperation.isCancelled {
                if !self.readyToWrite {
                    self.doDisconnect(Starscream_WSError(type: .outputStreamWriteError, message: "output stream had an error during write", code: 0))
                    break
                }
    ...
    private func doDisconnect(_ error: Error?) {
        ...
        callbackQueue.async { [weak self] in
            ...
            let userInfo = error.map{ [Starscream_WebsocketDisconnectionErrorKeyName: $0] }
            NotificationCenter.default.post(name: NSNotification.Name(Starscream_WebsocketDidDisconnectNotification), object: self, userInfo: userInfo)
        }
    }
