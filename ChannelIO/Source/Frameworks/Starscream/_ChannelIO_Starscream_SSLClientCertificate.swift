...
struct Starscream_SSLClientCertificateError: LocalizedError {
...
class Starscream_SSLClientCertificate {
    ...
    init(pkcs12Url: URL, importOptions: CFDictionary) throws {
        do {
            ...
            guard importStatus == errSecSuccess else {
                throw Starscream_SSLClientCertificateError(errorDescription: "(Starscream) Error during 'SecPKCS12Import', see 'SecBase.h' - OSStatus: \(importStatus)")
            }
            guard let identitiyAndCertificate = (rawIdentitiesAndCertificates as? Array<Dictionary<String, Any>>)?.first else {
                throw Starscream_SSLClientCertificateError(errorDescription: "(Starscream) Error - PKCS12 file is empty")
            }
            ...
            guard copyStatus == errSecSuccess else {
                throw Starscream_SSLClientCertificateError(errorDescription: "(Starscream) Error during 'SecIdentityCopyCertificate', see 'SecBase.h' - OSStatus: \(copyStatus)")
            }
            self.streamSSLCertificates = NSArray(objects: identity, identityCertificate!)
        } catch {
            throw error
