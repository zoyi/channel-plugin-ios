...
protocol Starscream_SSLTrustValidator {
    func isValid(_ trust: SecTrust, domain: String?) -> Bool
}

class Starscream_SSLCert {
    var certData: Data?
    var key: SecKey?
...
class Starscream_SSLSecurity : Starscream_SSLTrustValidator {
    ...
    */
    convenience init(usePublicKeys: Bool = false) {
        ...
        let certs = paths.reduce([Starscream_SSLCert]()) { (certs: [Starscream_SSLCert], path: String) -> [Starscream_SSLCert] in
            var certs = certs
            if let data = NSData(contentsOfFile: path) {
                certs.append(Starscream_SSLCert(data: data as Data))
            }
            return certs
        }
    ...
    - returns: a representation security object to be used with
    */
    init(certs: [Starscream_SSLCert], usePublicKeys: Bool) {
        self.usePublicKeys = usePublicKeys
        
        if self.usePublicKeys {
            DispatchQueue.global(qos: .default).async {
                let pubKeys = certs.reduce([SecKey]()) { (pubKeys: [SecKey], cert: Starscream_SSLCert) -> [SecKey] in
                    var pubKeys = pubKeys
            ...
            let certificates = certs.reduce([Data]()) { (certificates: [Data], cert: Starscream_SSLCert) -> [Data] in
