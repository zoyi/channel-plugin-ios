...
@interface _ChannelIO_MGSwipeTableInputOverlay : UIView
@property (nonatomic, weak) _ChannelIO_MGSwipeTableCell * currentCell;
@end

@implementation _ChannelIO_MGSwipeTableInputOverlay
...
@interface _ChannelIO_MGSwipeButtonsView : UIView
@property (nonatomic, weak) _ChannelIO_MGSwipeTableCell * cell;
@property (nonatomic, strong) UIColor * backgroundColorCopy;
@end

@implementation _ChannelIO_MGSwipeButtonsView
{
    NSArray * _buttons;
    UIView * _container;
    BOOL _fromLeft;
    ...
    _ChannelIO_MGSwipeDirection _direction;
    _ChannelIO_MGSwipeExpansionLayout _expansionLayout;
...
-(instancetype) initWithButtons:(NSArray*) buttonsArray direction:(_ChannelIO_MGSwipeDirection) direction swipeSettings:(_ChannelIO_MGSwipeSettings*) settings safeInset: (CGFloat) safeInset
{
...
-(void) expandToOffset:(CGFloat) offset settings:(_ChannelIO_MGSwipeExpansionSettings*) settings
{
...
-(void) transition:(_ChannelIO_MGSwipeTransition) mode percent:(CGFloat) t
{
    switch (mode) {
...
@implementation _ChannelIO_MGSwipeSettings
-(instancetype) init
{
    if (self = [super init]) {
        ...
        self.showAnimation = [[_ChannelIO_MGSwipeAnimation alloc] init];
        self.hideAnimation = [[_ChannelIO_MGSwipeAnimation alloc] init];
        self.stretchAnimation = [[_ChannelIO_MGSwipeAnimation alloc] init];
    }
    return self;
}
...
@implementation _ChannelIO_MGSwipeExpansionSettings
-(instancetype) init
{
    if (self = [super init]) {
        self.buttonIndex = -1;
        ...
        self.triggerAnimation = [[_ChannelIO_MGSwipeAnimation alloc] init];
    }
    return self;
}
@end

@interface _ChannelIO_MGSwipeAnimationData : NSObject
...
@property (nonatomic, strong) _ChannelIO_MGSwipeAnimation * animation;

@end

@implementation _ChannelIO_MGSwipeAnimationData
@end
...
@implementation _ChannelIO_MGSwipeAnimation
...
@implementation _ChannelIO_MGSwipeTableCell
{
  ...
  _ChannelIO_MGSwipeButtonsView * _leftView;
  _ChannelIO_MGSwipeButtonsView * _rightView;
    bool _allowSwipeRightToLeft;
    bool _allowSwipeLeftToRight;
    __weak _ChannelIO_MGSwipeButtonsView * _activeExpansion;

  _ChannelIO_MGSwipeTableInputOverlay * _tableInputOverlay;
  ...
  _ChannelIO_MGSwipeAnimationData * _animationData;
    void (^_animationCompletion)(BOOL finished);
    CADisplayLink * _displayLink;
    _ChannelIO_MGSwipeState _firstSwipeState;
}
...
-(void) initViews: (BOOL) cleanButtons
{
    if (cleanButtons) {
        ...
        _leftSwipeSettings = [[_ChannelIO_MGSwipeSettings alloc] init];
        _rightSwipeSettings = [[_ChannelIO_MGSwipeSettings alloc] init];
        _leftExpansion = [[_ChannelIO_MGSwipeExpansionSettings alloc] init];
        _rightExpansion = [[_ChannelIO_MGSwipeExpansionSettings alloc] init];
    }
    _animationData = [[_ChannelIO_MGSwipeAnimationData alloc] init];
...
-(void) createSwipeViewIfNeeded
{
    ...
    if (!_leftView && _leftButtons.count > 0) {
        ...
        _leftView = [[_ChannelIO_MGSwipeButtonsView alloc] initWithButtons:_leftButtons direction:MGSwipeDirectionLeftToRight swipeSettings:_leftSwipeSettings safeInset:safeInsets.left];
        _leftView.cell = self;
    ...
    if (!_rightView && _rightButtons.count > 0) {
        ...
        _rightView = [[_ChannelIO_MGSwipeButtonsView alloc] initWithButtons:_rightButtons direction:MGSwipeDirectionRightToLeft swipeSettings:_rightSwipeSettings safeInset:safeInsets.right];
...
- (void) showSwipeOverlayIfNeeded
{
    ...
    if (!_allowsMultipleSwipe) {
        ...
        _tableInputOverlay = [[_ChannelIO_MGSwipeTableInputOverlay alloc] initWithFrame:table.bounds];
...
-(void) updateState: (_ChannelIO_MGSwipeState) newState;
{
...
- (void)setSwipeOffset:(CGFloat) newOffset;
{
  ...
  _ChannelIO_MGSwipeButtonsView * activeButtons = sign < 0 ? _rightView : _leftView;
    _ChannelIO_MGSwipeSettings * activeSettings = sign < 0 ? _rightSwipeSettings : _leftSwipeSettings;
  ...
  _ChannelIO_MGSwipeButtonsView* but[2] = {_leftView, _rightView};
    _ChannelIO_MGSwipeSettings* settings[2] = {_leftSwipeSettings, _rightSwipeSettings};
    _ChannelIO_MGSwipeExpansionSettings * expansions[2] = {_leftExpansion, _rightExpansion};
    
    for (int i = 0; i< 2; ++i) {
      _ChannelIO_MGSwipeButtonsView * view = but[i];
        if (!view) continue;
...
{
    _ChannelIO_MGSwipeAnimation * animation = animated ? (_swipeOffset > 0 ? _leftSwipeSettings.hideAnimation: _rightSwipeSettings.hideAnimation) : nil;
...
-(void) showSwipe: (_ChannelIO_MGSwipeDirection) direction animated: (BOOL) animated
{
    [self showSwipe:direction animated:animated completion:nil];
}

-(void) showSwipe: (_ChannelIO_MGSwipeDirection) direction animated: (BOOL) animated completion:(void(^)(BOOL finished)) completion
{
    [self createSwipeViewIfNeeded];
    ...
    if (buttonsView) {
        ...
        _ChannelIO_MGSwipeAnimation * animation = animated ? (direction == MGSwipeDirectionLeftToRight ? _leftSwipeSettings.showAnimation : _rightSwipeSettings.showAnimation) : nil;
        [self setSwipeOffset:buttonsView.bounds.size.width * s animation:animation completion:completion];
    }
}

-(void) expandSwipe: (_ChannelIO_MGSwipeDirection) direction animated: (BOOL) animated
{
    CGFloat s = direction == MGSwipeDirectionLeftToRight ? 1.0 : -1.0;
    _ChannelIO_MGSwipeExpansionSettings* expSetting = direction == MGSwipeDirectionLeftToRight ? _leftExpansion : _rightExpansion;
    ...
    if(!_activeExpansion && expSetting.fillOnTrigger) {
        ...
        if (buttonsView) {
            __weak _ChannelIO_MGSwipeButtonsView * expansionView = direction == MGSwipeDirectionLeftToRight ? _leftView : _rightView;
            __weak _ChannelIO_MGSwipeTableCell * weakself = self;
...
{
    _ChannelIO_MGSwipeAnimation * animation = animated ? [[_ChannelIO_MGSwipeAnimation alloc] init] : nil;
    [self setSwipeOffset:offset animation:animation completion:completion];
}

-(void) setSwipeOffset:(CGFloat)offset animation: (_ChannelIO_MGSwipeAnimation *) animation completion:(void(^)(BOOL finished)) completion
{
    if (_displayLink) {
        [_displayLink invalidate];
        _displayLink = nil;
...
-(void) panHandler: (UIPanGestureRecognizer *)gesture
{
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        if (!_allowsMultipleSwipe) {
            ...
            for (_ChannelIO_MGSwipeTableCell * cell in cells) {
                if ([cell isKindOfClass:[_ChannelIO_MGSwipeTableCell class]] && cell != self) {
        ...
        __weak _ChannelIO_MGSwipeButtonsView * expansion = _activeExpansion;
        if (expansion) {
            __weak UIView * expandedButton = [expansion getExpandedButton];
            _ChannelIO_MGSwipeExpansionSettings * expSettings = _swipeOffset > 0 ? _leftExpansion : _rightExpansion;
            ...
            _ChannelIO_MGSwipeSettings * settings = _swipeOffset > 0 ? _leftSwipeSettings : _rightSwipeSettings;
            _ChannelIO_MGSwipeAnimation * animation = nil;
            if (_targetOffset == 0) {
