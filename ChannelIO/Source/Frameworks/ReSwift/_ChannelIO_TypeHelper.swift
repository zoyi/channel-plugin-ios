 ...
 */
@discardableResult
func ReSwift_withSpecificTypes<SpecificStateType, Action>(
        _ action: Action,
        state genericStateType: ReSwift_StateType?,
        function: (_ action: Action, _ state: SpecificStateType?) -> SpecificStateType
    ) -> ReSwift_StateType {
        guard let genericStateType = genericStateType else {
            return function(action, nil) as! ReSwift_StateType
        }
        ...
        return function(action, specificStateType) as! ReSwift_StateType
}
