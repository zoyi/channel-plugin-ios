 ...
 */
protocol ReSwift_StoreType: ReSwift_DispatchingStoreType {

    associatedtype State: ReSwift_StateType
     ...
     */
    var dispatchFunction: ReSwift_DispatchFunction! { get }
     ...
     */
    func subscribe<S: ReSwift_StoreSubscriber>(_ subscriber: S) where S.StoreSubscriberStateType == State
     ...
     */
    associatedtype ActionCreator = (_ state: State, _ store: ReSwift_StoreType) -> ReSwift_Action?

    /// AsyncActionCreators allow the developer to wait for the completion of an async action.
    associatedtype AsyncActionCreator =
        (_ state: State, _ store: ReSwift_StoreType,
