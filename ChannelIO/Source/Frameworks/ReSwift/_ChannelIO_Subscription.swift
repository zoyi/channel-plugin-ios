...
class ReSwift_SubscriptionBox<State>: Hashable {

    private let originalSubscription: ReSwift_Subscription<State>
    weak var subscriber: ReSwift_AnyStoreSubscriber?
    ...
    static func == (left: ReSwift_SubscriptionBox<State>, right: ReSwift_SubscriptionBox<State>) -> Bool {
...
class ReSwift_Subscription<State> {

    private func _select<Substate>(
        _ selector: @escaping (State) -> Substate
        ) -> ReSwift_Subscription<Substate>
    {
        return ReSwift_Subscription<Substate> { sink in
    ...
    init(sink: @escaping (@escaping (State?, State) -> Void) -> Void) {
    ...
    func skipRepeats(_ isRepeat: @escaping (_ oldState: State, _ newState: State) -> Bool)
        -> ReSwift_Subscription<State> {
        return ReSwift_Subscription<State> { sink in
            self.observer = { oldState, newState in
...
extension ReSwift_Subscription where State: Equatable {
    func skipRepeats() -> ReSwift_Subscription<State>{
        return self.skipRepeats(==)
    }
}

/// Subscription skipping convenience methods
extension ReSwift_Subscription {
    ...
    /// - parameter oldState: The store's old state, before the action is reduced.
    /// - parameter newState: The store's new state, after the action has been reduced.
    func skip(when: @escaping (_ oldState: State, _ newState: State) -> Bool) -> ReSwift_Subscription<State> {
    ...
    /// - parameter newState: The store's new state, after the action has been reduced.
    /// the subscriber.
    func only(when: @escaping (_ oldState: State, _ newState: State) -> Bool) -> ReSwift_Subscription<State> {
